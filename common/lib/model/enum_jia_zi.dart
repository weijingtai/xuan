...
enum NaYinFiveXing {
  ...
  CHENG_TOU_TU("城头土"),
  BAI_LA_JIN("白蜡金"),
  YANG_LIU_MU("杨柳木"),QUAN_ZHONG_SHUI("泉中水"),
  WU_SHANG_TU("屋上土"),
  PI_LI_HUO("霹雳火"),
  SONG_BAI_MU("松柏木"),
  CHANG_LIU_SHUI("长流水"),
...
enum JiaZi {
  ...
  NaYinFiveXing getNaYin() => this.naYin;
  String get naYinStr => this.naYin.name;
  (TianGan, DiZhi) get ganZhi => (this.tianGan, this.diZhi);
  String get name=> this.tianGan.value + this.diZhi.value;
  TianGan get gan => this.tianGan;
  DiZhi get zhi => this.diZhi;
  String get ganZhiStr => this.tianGan.value + this.diZhi.value;
  ...
  JiaZi getXunHeader(){
    if (number <= JiaZi.GUI_YOU.number){
      return JiaZi.JIA_ZI;
    }else if (number > JiaZi.GUI_YOU.number && number <= JiaZi.GUI_WEI.number){
      return JiaZi.JIA_XU;
      ...
      return JiaZi.JIA_WU;
    }else if (number > JiaZi.GUI_MAO.number && number <= JiaZi.GUI_CHOU.number){
  ...
  Tuple2<DiZhi,DiZhi> getKongWang(){
    if (number <= JiaZi.GUI_YOU.number){
      return Tuple2(DiZhi.XU, DiZhi.HAI);
    }else if (number > JiaZi.GUI_YOU.number && number <= JiaZi.GUI_WEI.number){
      return Tuple2(DiZhi.SHEN, DiZhi.YOU);
    }else if (number > JiaZi.GUI_WEI.number && number <= JiaZi.GUI_SI.number){
      return Tuple2(DiZhi.WU, DiZhi.WEI);
    }else if(number > JiaZi.GUI_SI.number && number <= JiaZi.GUI_MAO.number){
      return Tuple2(DiZhi.CHEN, DiZhi.SI);
    }else if (number > JiaZi.GUI_MAO.number && number <= JiaZi.GUI_CHOU.number){
      return Tuple2(DiZhi.YIN, DiZhi.MAO);
    } else {
      return Tuple2(DiZhi.ZI, DiZhi.CHOU);
    }
  }
  ...
  static JiaZi? getFromGanZhiValue(String value) {
    ...
    return gan != null && zhi != null ? JiaZiIndexMapper.jiaZiMapper[gan]![zhi] : null;
  }
  ...
  static List<JiaZi> getTenXunByXunHeader(JiaZi xunHeadJiaZi) {
   if (xunHeadJiaZi == JiaZi.JIA_ZI) {
     return [JIA_ZI, YI_CHOU, BING_YIN, DING_MAO, WU_CHEN, JI_SI, GENG_WU, XIN_WEI, REN_SHEN, GUI_YOU,];
   }else if(xunHeadJiaZi == JiaZi.JIA_XU){
     return [JIA_XU, YI_HAI, BING_ZI, DING_CHOU, WU_YIN, JI_MAO, GENG_CHEN, XIN_SI, REN_WU, GUI_WEI];
   } else if (xunHeadJiaZi == JiaZi.JIA_SHEN){
     return [JIA_SHEN, YI_YOU, BING_XU, DING_HAI, WU_ZI, JI_CHOU, GENG_YIN, XIN_MAO, REN_CHEN, GUI_SI,];
   }else if (xunHeadJiaZi == JiaZi.JIA_WU){
     return [JIA_WU, YI_WEI, BING_SHEN, DING_YOU, WU_XU, JI_HAI, GENG_ZI, XIN_CHOU, REN_YIN, GUI_MAO,];
   } else if (xunHeadJiaZi == JiaZi.JIA_CHEN){
     return [JIA_CHEN, YI_SI, BING_WU, DING_WEI, WU_SHEN, JI_YOU, GENG_XU, XIN_HAI, REN_ZI, GUI_CHOU,];
   }else if (xunHeadJiaZi == JiaZi.JIA_YIN){
     return [JIA_YIN,YI_MAO, BING_CHEN, DING_SI, WU_WU, JI_WEI, GENG_SHEN, XIN_YOU, REN_XU, GUI_HAI];
   }else {
