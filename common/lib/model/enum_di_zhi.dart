...
enum DiZhi {
  ...
  static Set<DiZhi> get fourMuYu{ // 沐浴之地
    return {ZI,WU,MAO,YOU};
  }
  static Set<DiZhi> get fourYiMa{ // 四驿马
    return {YIN,SHEN,SI,HAI};
  }
  static Set<DiZhi> get fourMuKu { // 四墓库
    // 墓库之地
    return {CHEN,XU,CHOU,WEI};
  }
  MonthToken get asMonthToken=>MonthToken.fromDiZhi(this);
  bool get isSelfXing => [DiZhi.CHEN,DiZhi.WU,DiZhi.YOU,DiZhi.HAI].contains(this);
  // {
...
enum DiZhiXing {
  ZI_MAO(name: "子卯",
      nickname1: "无礼之刑",
      content: const [DiZhi.ZI, DiZhi.MAO]),
  // 固定刑克顺序
  YIN_SI_SHEN(name: "寅巳申",
      nickname1: "无恩之刑",
      content: const [DiZhi.YIN, DiZhi.SI, DiZhi.SHEN]),
  CHOU_XU_WEI(name: "丑戌未",
      nickname1: "持势之刑",
      content: const [DiZhi.CHOU, DiZhi.XU, DiZhi.WEI]),
  CHEN_CHEN(name: "辰辰", nickname1: "辰辰自刑", content: const [DiZhi.CHEN]),
  WU_WU(name: "午午", nickname1: "午午自刑", content: const [DiZhi.WU]),
  YOU_YOU(name: "酉酉", nickname1: "酉酉自刑", content: const [DiZhi.YOU]),
  HAI_HAI(name: "亥亥", nickname1: "亥亥自刑", content: const [DiZhi.HAI]);
  ...
  static DiZhiXing? getFromValue(String value) {
    ...
    Set<String> _set = value.split("").toSet();
    Set<String> allDiZhiSet = DiZhi.listAll.map((d) => d.value).toSet();
    if (_set.any((e) => !allDiZhiSet.contains(e))) {
      throw Exception("DiZhiXing value is not valid with value:$value");
    }

    Set<DiZhi> _diZhiSet = _set.map((e) => DiZhi.getFromValue(e)!).toSet();
    return getFromValueDiZhi(_diZhiSet);
  }
  ...
  static DiZhiXing? getFromValueDiZhi(Set<DiZhi> diZhiSet) {
    ...
    if (diZhiSet.length == 1) {
      ...
      if (diZhiSet.contains(DiZhi.YIN) && diZhiSet.contains(DiZhi.SI) &&
          diZhiSet.contains(DiZhi.SHEN)) {
        return DiZhiXing.YIN_SI_SHEN;
      } else if (diZhiSet.contains(DiZhi.XU) && diZhiSet.contains(DiZhi.CHOU) &&
...
enum DiZhiChong{
  // 地支六冲
  ZI_WU(name:"子午",content: const {DiZhi.ZI,DiZhi.WU}),
  MAO_YOU(name:"卯酉",content: const {DiZhi.MAO,DiZhi.YOU}),
  YIN_SHEN(name:"寅申",content: const {DiZhi.YIN,DiZhi.SHEN}),
  SI_HAI(name:"巳亥",content: const {DiZhi.SI,DiZhi.HAI}),
  CHEN_XU(name:"辰戌",content: const {DiZhi.CHEN,DiZhi.XU}),
  CHOU_WEI(name:"丑未",content: const {DiZhi.CHOU,DiZhi.WEI});
  ...
  static DiZhi getOtherDiZhi(DiZhi single){
     ...
     getFromSingleDiZhi(single)!.content.forEach((e){
       if (e != single){
         res = e;
       }
     });
  ...
  static DiZhiChong? getFromValue(String diZhiStr) {
     ...
     // 检查是否全部是地支
      Set<String> _set= diZhiStr.split("").toSet();
     if (_set.length == 1){
         return DiZhiChong.getFromSingleDiZhi(DiZhi.getFromValue(_set.first)!);
     }
    if(!DiZhi.listAll.map((d)=>d.value).toSet().containsAll(_set)){
      return throw Exception("DiZhiChong value is not valid with value:$diZhiStr");
    }

     Set<DiZhi> _diZhiSet =_set.map((e)=>DiZhi.getFromValue(e)!).toSet();

     return getFromDiZhi(_diZhiSet.first,_diZhiSet.last);
...
enum DiZhiPo{
  // 地支六破
  ZI_YOU(name:"子酉",content: const {DiZhi.ZI,DiZhi.YOU}),
  MAO_WU(name:"卯午",content: const {DiZhi.MAO,DiZhi.WU}),

  YIN_HAI(name:"寅亥",content: const {DiZhi.YIN,DiZhi.HAI}),
  SI_SHEN(name:"巳申",content: const {DiZhi.SI,DiZhi.SHEN}),

  WEI_XU(name:"未戌",content: const {DiZhi.WEI,DiZhi.XU}),
  CHOU_CHEN(name:"丑辰",content: const {DiZhi.CHOU,DiZhi.CHEN});
  ...
  static DiZhiPo? getFromValue(String diZhiStr) {
    ...
    Set<String> _set= diZhiStr.split("").toSet();
    if (_set.length == 1){
      return DiZhiPo.getFromSingleDiZhi(DiZhi.getFromValue(_set.first)!);
    }
    if(!DiZhi.listAll.map((d)=>d.value).toSet().containsAll(_set)){
      return throw Exception("DiZhiChong value is not valid with value:$diZhiStr");
    }
...
enum DiZhiHai{
  // 地支六害
  ZI_WEI(name:"子未",content: const {DiZhi.ZI,DiZhi.WEI}),
  CHOU_WU(name:"丑午",content: const {DiZhi.CHOU,DiZhi.WU}),

  YIN_SI(name:"寅巳",content: const {DiZhi.YIN,DiZhi.SI}),
  MAO_CHEN(name:"卯辰",content: const {DiZhi.MAO,DiZhi.CHEN}),

  SI_HAI(name:"申亥",content: const {DiZhi.SI,DiZhi.HAI}),
  YOU_XU(name:"酉戌",content: const {DiZhi.YOU,DiZhi.XU});
  ...
  static DiZhiHai? getFromValue(String diZhiStr) {
    ...
    Set<String> _set= diZhiStr.split("").toSet();
    if (_set.length == 1){
      return DiZhiHai.getFromSingleDiZhi(DiZhi.getFromValue(_set.first)!);
    }
    if(!DiZhi.listAll.map((d)=>d.value).toSet().containsAll(_set)){
      return throw Exception("DiZhiChong value is not valid with value:$diZhiStr");
    }
...
enum DiZhiSanHui{
  // 三会
  HAI_ZI_CHOU(name:"亥子丑", content: {DiZhi.HAI,DiZhi.ZI,DiZhi.CHOU},fiveXing: FiveXing.SHUI),
  YIN_MAO_CHEN(name:"寅卯辰", content: {DiZhi.YIN,DiZhi.MAO,DiZhi.CHEN},fiveXing: FiveXing.MU),
  SI_WU_WEI(name:"巳午未", content: {DiZhi.SI,DiZhi.WU,DiZhi.WEI},fiveXing: FiveXing.HUO),
  SHEN_YOU_XU(name:"申酉戌", content: {DiZhi.SHEN,DiZhi.YOU,DiZhi.XU},fiveXing: FiveXing.JIN);
  ...
  static DiZhiSanHui? getFromDiZhiStr(String diZhiStr) {
    ...
    List<DiZhi> _diZhiSet = _set.map((e)=>DiZhi.getFromValue(e)!).toList();


    return getFromDiZhi(_diZhiSet[0],_diZhiSet[1],_diZhiSet[2]);
  }
...
enum DiZhiSanHe{
  // 三会
  SHEN_ZI_CHEN(name:"申子辰", content: {DiZhi.SHEN,DiZhi.ZI,DiZhi.CHEN},fiveXing: FiveXing.SHUI),
  HAI_MAO_WEI(name:"亥卯未", content: {DiZhi.HAI,DiZhi.MAO,DiZhi.WEI},fiveXing: FiveXing.MU),
  YIN_WU_XU(name:"寅午戌", content: {DiZhi.YIN,DiZhi.WU,DiZhi.XU},fiveXing: FiveXing.HUO),
  SI_YOU_CHOU(name:"巳酉丑", content: {DiZhi.SI,DiZhi.YOU,DiZhi.CHOU},fiveXing: FiveXing.JIN);
  ...
  static DiZhi getHorseBySingleDiZhi(DiZhi diZhi){
    ...
    if (res == SHEN_ZI_CHEN){
      return DiZhi.YIN;
    }else if (res == HAI_MAO_WEI){
      return DiZhi.SI;
    }else if (res == YIN_WU_XU){
      return DiZhi.SHEN;
    }else {
      return DiZhi.HAI;
    }
