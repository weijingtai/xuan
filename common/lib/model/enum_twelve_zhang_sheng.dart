...
enum TwelveZhangSheng{
  ...
  static TwelveZhangSheng fromIndex(int index) => TwelveZhangSheng.values[index];
  static TwelveZhangSheng fromName(String name) => TwelveZhangSheng.values.firstWhere((element) => element.name == name);
  static TwelveZhangSheng getZhangShengByTianGanDiZhi(TianGan tianGan,DiZhi diZhi){
    var index = zhangShengMapper[tianGan]!.indexOf(diZhi.name)!;
    return TwelveZhangSheng.fromIndex(index);
  }
  bool get isStrong =>{ZHANG_SHEN,MU_YU,GUAN_DAI,LIN_GUAN,DI_WANG}.contains(this);
  bool get isWeak =>{SHUAI,BING,SI,MU,JUE,TAI,YANG}.contains(this);

  List<TwelveZhangSheng> get listStrong => [ZHANG_SHEN,MU_YU,GUAN_DAI,LIN_GUAN,DI_WANG];
