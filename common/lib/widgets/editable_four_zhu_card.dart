...
class _EditableFourZhuCardState extends State<EditableFourZhuCard> {
      ...
      children: [
        ...
        Row(
          children: [
            ...
            // Column headers with overlay drop zones to avoid width consumption
            Expanded(
              child: LayoutBuilder(
                builder: (ctx, constraints) {
                  ...
                  // Header cells row (fixed widths per column)
                  final headerRow = Row(
                    children: _pillars.asMap().entries.map((entry) {
                      ...
                      if (!widget.isEditable) {
                        return SizedBox(width: cellW, child: headerCell);
                      }
                      ...
                      return SizedBox(
                        ...
                        child: LongPressDraggable<int>(
                          ...
                          child:
                              isDragging ? const SizedBox.shrink() : headerCell,
                        ),
                      );
                    }).toList(),
                  );

                  // Overlay drop zones positioned at column boundaries
                  final overlay = Stack(
                    children: [
                      if (_hoverDropIndex != null)
                        Positioned(
                          left: (_hoverDropIndex! * cellW)
                              .clamp(0.0, contentWidth - cellW),
                          width: cellW,
                          top: 0,
                          bottom: 0,
                          child: IgnorePointer(
                      ...
                      if (widget.isEditable)
                        ...List.generate(count + 1, (i) {
                          final left = (i * cellW - dzWidth / 2)
                              .clamp(0.0, contentWidth - dzWidth);
                          return Positioned(
                            left: left,
                            width: dzWidth,
                            top: 0,
                            bottom: 0,
                            child: _buildPillarDropZone(i, dzWidth),
                          );
                        }),
                    ],
                  );

                  // Only accept pointer events during active drag so header cells can start drag.
                  final overlayWidget = IgnorePointer(
                  ...
                  return Stack(children: [
                    headerRow,
                    if (widget.isEditable) overlayWidget
                  ]);
                },
              ),
            ),
    ...
    return DragTarget<String>(
      onWillAcceptWithDetails: (details) {
        ...
        if (data.startsWith('row_')) {
          setState(() => _hoverDropIndex =
              position + 1000); // Offset to distinguish from column zones
          return true;
        }
        return false;
      },
      ...
      builder: (context, candidateData, rejectedData) {
        final show =
            _hoverDropIndex == position + 1000 || candidateData.isNotEmpty;
        return AnimatedContainer(
          duration: const Duration(milliseconds: 150),
          height: show ? 40 : 4,
          margin: const EdgeInsets.symmetric(vertical: 2),
                    ...
                    LongPressDraggable<int>(
                      ...
                      feedback: Material(
                        ...
                        child: ConstrainedBox(
                          ...
                          child: Opacity(
                              opacity: 0.95,
                              child: _buildHorizontalPillarTile(
                                  context,
                                  idx,
                                  pillar,
                                  visibleRows,
  ...
  Widget _horizontalPlaceholderTile(
      BuildContext context, double tileH, double tileW) {
      ...
      child: Container(
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(
                widget.metricsResolver.cornerRadius(context)),
            border: Border.all(color: border)),
  ...
  Widget _buildHorizontalSlot(
      int position, double slotW, double tileH, double tileW) {
    final active = _hoverDropIndex == position;
  ...
  Widget _buildVerticalHeader(BuildContext context) {
    ...
    return Row(
      children: [
        SizedBox(
            width: 48,
            child: Text('四柱', style: labelStyle, textAlign: TextAlign.center)),
  ...
  Widget _buildVerticalPillarTile(
      BuildContext context, PillarType pillar, List<RowConfig> visibleRows) {
    final border = Theme.of(context).dividerColor;
    final labelStyle = widget.styleResolver.resolveTextStyle(
      context: context,
      rowType: RowType.heavenlyStem,
      ...
      child: Row(
        ...
        children: [
          Icon(Icons.drag_indicator,
              size: 18, color: widget.isEditable ? null : Colors.transparent),
          const SizedBox(width: 8),
          SizedBox(
              width: 36,
              child: Text(pillarLabel,
                  ...
                  Row(
                    children: [
                      ...
                      ..._pillars.asMap().entries.expand((entry) {
                        ...
                        final p = entry.value;
                        final label = widget.pillarLabelResolver?.call(p) ??
                            _defaultPillarLabel(p);
                        final headerCell = widget.pillarHeaderBuilder?.call(
                              context,
                              pillarType: p,
                        ...
                        final draggable = widget.isEditable
                            ? LongPressDraggable<int>(
                                ...
                                feedback: Material(
                                  ...
                                  child: ConstrainedBox(
                                    constraints:
                                        BoxConstraints.tightFor(width: cellW),
                                    child: Opacity(
                                        opacity: 0.85, child: headerCell),
                                  ),
                                ),
                                childWhenDragging:
                                    _placeholderColumn(context, cellW),
                                child: cell,
                              )
                            : cell;
                        return [
      ...
      child: Stack(
        ...
        children: [
          ...
          DragTarget<int>(
            onWillAccept: (from) {
              final accept =
                  from != null && from >= 0 && from < _pillars.length;
              if (accept) setState(() => _hoverDropIndex = position);
              return accept;
            },
            onLeave: (_) {
              if (_hoverDropIndex == position)
                setState(() => _hoverDropIndex = null);
            },
            onAccept: (from) {
    ...
    for (var i = 0; i < visible.length; i++) {
      ...
      if (i != visible.length - 1) {
        final divider = widget.dividerBuilder?.call(context) ??
            widget.styleResolver.resolveRowDivider(
                context: context, cardStyle: widget.cardStyle, rowConfig: cfg);
                              ...
                              return Positioned(
                                ...
                                child: DragTarget<PillarType>(
                                  onWillAccept: (data) {
                                    final accept = data != null;
                                    if (accept)
                                      setState(() => _hoverDropIndex = i);
                                    return accept;
                                  },
                                  onLeave: (_) {
                                    if (_hoverDropIndex == i)
                                      setState(() => _hoverDropIndex = null);
                                  },
                                  onAccept: (type) {
