...
class CardRow {
  static const String pillarHeader = 'pillarHeader';
  static const String tianGan = 'tianGan';
  static const String diZhi = 'diZhi';
  static const String xunShou = 'xunShou';
  static const String naYin = 'naYin';
  static const String kongWang = 'kongWang';
}

class RowReorderableCard extends StatelessWidget {
  final String? title;
  final Map<String, JiaZi> pillars;
  final List<String> rowOrder;
  final void Function(int oldIndex, int newIndex) onReorder;

  const RowReorderableCard({
    Key? key,
    this.title,
  ...
  Widget _buildPillarHeaderRow(BuildContext context) {
        ...
        Expanded(
          child: SizedBox(
            ...
            child: LayoutBuilder(builder: (context, constraints) {
              ...
              return Stack(
                children:
                    pillars.keys.toList().asMap().entries.map((indexedEntry) {
  ...
  Widget _buildTianGanRow(BuildContext context) {
    final theme = Theme.of(context);
    final labelColor = theme.textTheme.titleMedium?.color;
    return Row(
      children: [
  ...
  Widget _buildDiZhiRow(BuildContext context) {
    final theme = Theme.of(context);
    final labelColor = theme.textTheme.titleMedium?.color;
    return Row(
      children: [
  ...
  Widget _buildRow(BuildContext context, String rowType) {
    switch (rowType) {
      case CardRow.pillarHeader:
        return _buildPillarHeaderRow(context);
      case CardRow.tianGan:
        return _buildTianGanRow(context);
      case CardRow.diZhi:
        return _buildDiZhiRow(context);
      case CardRow.xunShou:
        return _buildInfoRow(context, '旬首', pillars,
            (jiazi) => jiazi.getXunHeader().ganZhiStr);
      case CardRow.naYin:
        return _buildInfoRow(
            context, '纳音', pillars, (jiazi) => jiazi.naYin.name);
      case CardRow.kongWang:
        return _buildInfoRow(context, '空亡', pillars, (jiazi) {
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ReorderableListView(
              shrinkWrap: true,
              onReorder: onReorder,
              children: rowOrder.map((rowType) {
              return Container(
                key: ValueKey('${title}_$rowType'),
                decoration: BoxDecoration(
                  border: Border(bottom: BorderSide(color: theme.dividerColor, width: 0.5)),
                ),
                child: _buildRow(context, rowType),
              );
            }).toList(),
          ),
          ),
