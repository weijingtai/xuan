...
class _ValueChip extends StatelessWidget {
  final String label;
  final double value;
  const _ValueChip({super.key, required this.label, required this.value});
  @override
  Widget build(BuildContext context) {
    return Container(
...
/// ColorfulTextStyleEditorWidgetV2
/// UI-only refactor of the original editor with the same behavior.
/// Keeps font, size, weight, color, shadow controls and dual-theme preview.
/// Parameters:
/// - [label]: Display name of the style being edited.
/// - [initialStyle]: Optional starting style; falls back to defaults.
/// - [onChanged]: Callback invoked with updated `TextStyle` on any change.
/// - [group]: Text group for preview (天干/地支等)；默认由 label 推断。
/// - [onPerCharPureColorChanged]: Per-character pure color changes.
/// - [onlyWheel], [dialogEnablePrimaryAccent], [showInlineWheel]: Color UI flags.
class ColorfulTextStyleEditorWidgetV2 extends StatefulWidget {
  final String label;
  final TextStyle? initialStyle;
  final ValueChanged<TextStyle> onChanged;
  final TextGroup? group;
  ...
  const ColorfulTextStyleEditorWidgetV2({
    super.key,
    required this.label,
    this.initialStyle,
    required this.onChanged,
  ...
  @override
  State<ColorfulTextStyleEditorWidgetV2> createState() =>
      _ColorfulTextStyleEditorWidgetV2State();
}

class _ColorfulTextStyleEditorWidgetV2State
    extends State<ColorfulTextStyleEditorWidgetV2> {
  // Font state
  late String _fontFamily;
  late double _fontSize;
  late FontWeight _fontWeight;
  // Uniform color (used in pure mode)
  Color _color = Colors.black87;

  // Preview key to read current ColorPreviewMode
  final GlobalKey<_DualThemeColorPreviewState> _previewKey =
      GlobalKey<_DualThemeColorPreviewState>();
  ...
  /// Initialize local state from `initialStyle`.
  /// Sets font, weight, size, color and extracts single-layer shadow values.
  /// Detects follow-char sentinel RGB for shadow color.
  @override
  void initState() {
    super.initState();
    final s = widget.initialStyle ?? const TextStyle();
  ...
  /// Emit updated `TextStyle` to the parent with the correct color semantics.
  /// - In colorful mode: send `color: null` to let the card apply element colors.
  /// - In pure mode: send the uniform `_color`.
  void _emit() {
    final previewState = _previewKey.currentState;
    final bool isColorfulMode = previewState != null
        ? previewState.mode == ColorPreviewMode.colorful
        : (widget.initialStyle?.color == null);
  ...
  /// 构建阴影预览盒，顶部水平滑块控制位移 X，右侧垂直滑块控制位移 Y。
  /// 增强：支持在预览区域拖拽调整位移；叠加十字准线与数值徽标；美化滑块样式。
  /// 返回一个包含“效果”文本的容器，用于直观演示阴影变化。
  Widget _buildShadowPreviewBox() {
    final TextStyle previewStyle = TextStyle(
      fontFamily: _fontFamily.isEmpty ? null : _fontFamily,
      fontSize: _fontSize,
      fontWeight: _fontWeight,
      color: _color,
            ...
            Positioned.fill(
              child: GestureDetector(
                onPanUpdate: (details) {
                  setState(() {
                    _shadowOffsetX =
                        (_shadowOffsetX + details.delta.dx / 2).clamp(-24, 24);
                    _shadowOffsetY =
                        (_shadowOffsetY + details.delta.dy / 2).clamp(-24, 24);
                  });
                  _emit();
                },
                child: Stack(
                    ...
                    Center(
                      child: Text('效果',
                          style: previewStyle.copyWith(
                            fontSize: (_fontSize * 1.2).clamp(14, 48),
                          )),
                    ),
                  ],
                ),
            ...
            Positioned(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  _ValueChip(label: 'X', value: _shadowOffsetX),
                ],
              ),
            ),
            ...
            Positioned(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 8),
                  _ValueChip(label: 'Y', value: _shadowOffsetY),
                ],
              ),
            ),
  ...
  /// Update uniform color from the preview's global pure color and emit.
  /// Parameters:
  /// - [picked]: New global pure color selected in preview.
  void _onPreviewGlobalPureColorChanged(Color picked) {
    setState(() => _color = picked);
    _emit();
  }

  /// Build the "字体" section: family dropdown, weight dropdown and size slider.
  /// Returns a vertical layout grouping font controls.
  Widget _buildFontSection() {
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            const SizedBox(width: 60, child: Text('字号')),
            Expanded(
              child: Slider(
                value: _fontSize.clamp(8, 72),
                min: 8,
            ...
            SizedBox(width: 40, child: Text('${_fontSize.toInt()}')),
          ],
        ),
        if (widget.showInlineWheel) ...[
          const SizedBox(height: 8),
  ...
  /// Build the "阴影" section UI（符合设计稿）。
  /// 行首右侧是开关；下方有预览盒（含位移X/Y滑块），再下方是模糊半径、阴影颜色、不透明度。
  Widget _buildShadowSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            const Text('阴影', style: TextStyle(fontWeight: FontWeight.w600)),
            const Spacer(),
            Switch(
              value: _shadowEnabled,
              onChanged: (v) {
        ...
        if (_shadowEnabled) ...[
          ...
          Row(
            children: [
              ...
              const Text('阴影颜色跟随字符颜色'),
            ],
          ),
          Row(
            children: [
              const SizedBox(width: 80, child: Text('模糊半径')),
              Expanded(
                child: Slider(
                  value: _shadowBlurRadius,
                  min: 0,
              ...
              SizedBox(
                width: 48,
                child: Text(_shadowBlurRadius.toStringAsFixed(0),
                    textAlign: TextAlign.right),
              ),
            ],
          ),
          if (!_shadowFollowCharColor)
            Row(
              children: [
                const SizedBox(width: 80, child: Text('阴影颜色')),
                InkWell(
                  onTap: () async {
                    final picked = await showColorPickerDialog(
                      context,
  ...
  /// Build V2 layout: Title, grouped sections for font/shadow, and theme preview.
  /// Returns the full editor widget tree.
  @override
  Widget build(BuildContext context) {
    final TextGroup group = widget.group ??
        (() {
          switch (widget.label) {
            case '天干':
              return TextGroup.tianGan;
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Builder(
                    builder: (_) {
                      final mode = _previewKey.currentState?.mode ??
                          (widget.initialStyle?.color == null
                              ? ColorPreviewMode.colorful
                              : ColorPreviewMode.pure);
                      return Text(
                          '当前：${mode == ColorPreviewMode.pure ? '纯色' : '彩色'}',
                          style: Theme.of(context)
                              .textTheme
                              .bodySmall
  ...
  Color _defaultColorForChar(String ch) {
    if (widget.group == TextGroup.tianGan) {
      if (_mode == ColorPreviewMode.colorful) {
        return ConstResourcesMapper
                .zodiacGanColors[TianGan.getFromValue(ch)!] ??
            Colors.grey;
      } else {
        return {
              TianGan.JIA: Colors.black87, // 铜绿
              TianGan.YI: Colors.black87, // 铜绿
              TianGan.BING: Colors.black87, // 西瓜红
              TianGan.DING: Colors.black87, // 西瓜红
              TianGan.WU: Colors.black87, // 驼色
              TianGan.JI: Colors.black87, // 驼色
              TianGan.GENG: Colors.black87, // 黄金叶
              TianGan.XIN: Colors.black87, // 黄金叶
              TianGan.REN: Colors.black87, //  景泰蓝
              TianGan.GUI: Colors.black87, // 景泰蓝
            }[TianGan.getFromValue(ch)!] ??
            Colors.grey;
      }
    } else {
