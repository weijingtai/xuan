...
class FlutterMapScreen extends StatefulWidget {
  final Location seekerAreaLocation;
  Coordinates? seekerCoordinate;
  Coordinates? seerCoordinate;
  FlutterMapScreen(this.seekerAreaLocation, {super.key,this.seerCoordinate,this.seekerCoordinate});
...
class _FlutterMapScreenState extends State<FlutterMapScreen>
  ...
  final Logger _l = Logger(level: Level.trace, printer: PrettyPrinter(errorMethodCount: 16));
  // 标记列表
  ...
  LatLng get locationCenterPoint => LatLng(widget.seekerAreaLocation.coordinates.latitude,widget.seekerAreaLocation.coordinates!.longitude);
  ...
  late LatLng startAnimationLatLng;
  late LatLng? _newSeekerPointCenter = null;
  ...
  void initState() {
    super.initState();
    seekerCoordinate = LatLng(widget.seekerAreaLocation.coordinates.latitude, widget.seekerAreaLocation.coordinates.longitude);

    if (widget.seerCoordinate != null){
      seerPoint = LatLng(widget.seerCoordinate!.latitude, widget.seerCoordinate!.longitude);
      // seerLocationMarker = buildSeerMarker(seerPoint);
      // centerMaker = buildAreaCenterMarker(locationCenterPoint);
    }
    if (widget.seekerCoordinate != null){
      _newSeekerPointCenter = LatLng(widget.seekerCoordinate!.latitude, widget.seekerCoordinate!.longitude);
    }
  ...
  TextStyle locationTextStyle = const TextStyle(
      height:1,fontSize: 14, color: Colors.black45, fontWeight: FontWeight.w600,fontFamily: "NotoSansSC",shadows: [
        Shadow(color: Colors.black38,)
  ]);
  TextStyle lngLatTextStyle =
  const TextStyle(fontSize: 14, color: Colors.black87,fontFamily: "NotoSansSC");
            ...
            Container(
              ...
              child: Container(
                ...
                child: Text.rich(
                  TextSpan(
                    text: "我的位置",
                      style: lngLatTextStyle.copyWith(color: Colors.black87,fontSize: 16,fontWeight: FontWeight.normal),
                )
                )
              ),
            )
  ...
  Marker? get seerMarker => seerPoint!=null? buildSeerMarker(seerPoint!):null;
  Marker get seekerAreaMarker => _buildSeekerAreaCenterMarker();
  Widget _buildSeekerAreaCenterMarkerIcon(){

    final Location location = widget.seekerAreaLocation;
    return Column(
        children: [
          Opacity(
                ...
                children: [
                  ...
                  Column(
                      ...
                      children: [
                        ...
                        Text.rich(
                            TextSpan(
                                text: "(${location.province.name} · ${location.city.name}",
                  ...
                  InkWell(
                    radius: 16,onTap: resetLocationCenterAsSeerLocation,
  ...
  Marker _buildSeekerAreaCenterMarker(){
    ...
    return  Marker(
        width: 256,
        height: iconSize+68,
        point: LatLng(widget.seekerAreaLocation.coordinates.latitude, widget.seekerAreaLocation.coordinates.longitude),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
                  ...
                  children: [
                    Text.rich(
                      TextSpan(
                          ...
                          children: [
                            TextSpan(
                                text:"${widget.seekerAreaLocation.lowestGeoLocation.coordinates.longitude}, ${widget.seekerAreaLocation.lowestGeoLocation.coordinates.latitude}"
                            ),
                            if (_mouseCircleCenter != null)
                              TextSpan(
                                  text:" -> ${_mouseCircleCenter!.longitude.toStringAsFixed(5)}, ${_mouseCircleCenter!.latitude.toStringAsFixed(5)}",style: TextStyle(color: Colors.blue)
                              )
                          ]
                      ),
                    ),
                    Text.rich(
                        TextSpan(
                            text: "${widget.seekerAreaLocation.province.name} · ${widget.seekerAreaLocation.city.name}",
                            style: locationTextStyle,
                            children: [
                              if (widget.seekerAreaLocation.area != null)
                                TextSpan(
                                  text:" · ${widget.seekerAreaLocation.area!.name}",
                                  // style: locationTextStyle,
                                ),
                              TextSpan(text:"（行政中心坐标）",style: locationTextStyle.copyWith(fontWeight: FontWeight.bold)),
                              // WidgetSpan(
                              //     child: )
                            ]
                        )
              ...
              children: [
                Text.rich(
                  TextSpan(
                      ...
                      children: [
                        TextSpan(
                            text:"${_newSeekerPointCenter!.longitude.toStringAsFixed(5)}, ${_newSeekerPointCenter!.latitude.toStringAsFixed(5)}"
                        )
                      ]
                  ),
                ),
          ...
          actions: [
            TextButton(onPressed: _newSeekerPointCenter==null?null
                :(){
              Navigator.pop(context,Coordinates(latitude: _newSeekerPointCenter!.latitude, longitude: _newSeekerPointCenter!.longitude));
            }, child: Text("确定"))
          ],
        ),
        body: buildMap(widget.seerCoordinate ?? widget.seekerAreaLocation.coordinates));
  }

  void _buildTimeZonePolygons(){
    fetchTimezonesGeoJSONFromLocal().then((val){
      GeoJSONFeatureCollection geoJSONFeatureCollection = (val as GeoJSONFeatureCollection);
      // geoJSONFeatureCollection.features.forEach((e) {
      //   print(e?.geometry?.type);
      // });
      List<GeoJSONFeature> list=geoJSONFeatureCollection.features.where((element) => element != null).map((e)=>e!).toList();
      _l.d(list.length);
      List<Polygon> _tmpPolygon = [];
      for (var f in list){
        // _l.d(f.geometry?.type);
        if (f.geometry!.type == GeoJSONType.polygon){
          final coordinates = (f.geometry as GeoJSONPolygon).coordinates[0] as List<dynamic>;
          List<LatLng> points = [];
          ...
          final multiCoordinates =  (f.geometry as GeoJSONMultiPolygon).coordinates as List<dynamic>;
  ...
  void _buildAreaPolygons(){
    fetchGeoJSONFromLocalStorage(widget.seekerAreaLocation).then((val){
      _polygons = val;
      _l.d(_polygons);
        setState(() {
        });
  ...
  Widget buildMap(Coordinates initCoordinates) {
    ...
    return Listener(
      ...
      child: FlutterMap(
        mapController: mapController,
        options: MapOptions(
          initialCenter: LatLng(initCoordinates.latitude,
        ...
        children: [
          ...
          _polygons == null? const SizedBox():PolygonLayer(polygons: _polygons!),
  ...
  Future<GeoJSON> fetchTimezonesGeoJSONFromLocal()async{
    ...
    // 从本地获取资源
    String geoJsonString = await rootBundle.loadString('assets/dataset/combined-with-oceans-now.json');
  ...
  Future<List<Polygon>> fetchGeoJSONFromLocalStorage(Location location)async{
    _l.i("fetchGeoJSON for ${location.lowestGeoLocation.name} by it's code:${location.lowestGeoLocation.code}");
    // 从本地获取资源
    String geoJsonString = await rootBundle.loadString('assets/dataset/geo/${location.lowestGeoLocation.code}.geojson');
    final geoJsonData = json.decode(geoJsonString);
    return convertGeoJSONToPoints(geoJsonData);
  }
  Future<List<Polygon>> fetchGeoJSONFromNetwork(Location location) async {
    _l.i("fetchGeoJSON for ${location.lowestGeoLocation.name} by it's code:${location.lowestGeoLocation.code}");
    final String urlPath = 'https://geo.datav.aliyun.com/areas_v3/bound/${location.lowestGeoLocation.code}.json';
    try {
      Dio dio = Dio();
      // 替换为 DataV.GeoAtlas 实际的 GeoJSON 数据链接
      Response response = await dio.get(urlPath);
      if (response.statusCode == 200) {
        _l.i("fetchGeoJSON for ${location.lowestGeoLocation.name} by it's code:${location.lowestGeoLocation.code} success.");
        final geoJsonData = response.data;
        return convertGeoJSONToPoints(geoJsonData);
      } else {
        _l.e("fetchGeoJSON for ${location.lowestGeoLocation.name} by it's code:${location.lowestGeoLocation.code} failed.");
  ...
  List<Polygon> convertTimeZonesGeoJSON2Points(Map<String,dynamic> geoJsonData){
  ...
  List<Polygon> doGeoJSONConvert(List<GeoJSON> geoJsonList){
    ...
    for (final geoJson in geoJsonList) {
      ...
      if (geoJson.type == GeoJSONType.polygon){
        final coordinates = (geoJson as GeoJSONPolygon).coordinates[0] as List<dynamic>;
        List<LatLng> points = [];
        coordinates.forEach((coord) {
          // 如果 lng 的大于 180.0则设置为180.0;
          final lng = coord[0] as double;
        ...
        _l.d("type is MultiPolygon");
        final multiCoordinates =  (geoJson as GeoJSONMultiPolygon).coordinates as List<dynamic>;
        for (final polygonCoordinates in multiCoordinates) {
          for (final ringCoordinates in polygonCoordinates) {
            List<LatLng> points = [];
            ringCoordinates.forEach((coord) {
