...
- [ ] UI 中设置卡是否放置在主卡片外侧，交互是否清晰？
- [ ] 交节方案（精度）与子时策略的耦合是否明示（时辰精度依赖日界规则）？
...
- 交节方案（精度）
  - [ ] `_sameBucket/_entered` 是否准确表达四种精度（秒/分钟/小时/时辰）？
  - [ ] 时辰精度下的“同桶进入”是否正确使用当前子时策略的日界规则？
  - [ ] 节气边界与物候边界均应用了精度判定，且“秒”模式不会提前进入；分钟/小时/时辰为“同桶进入”。

## 数据与状态
- [ ] `JieQiPhenologyStore` 与 `ZiStrategyStore` 的默认值与初始化时机（main.dart）正确。
- [ ] `SharedPreferences` Key 命名一致、读写健壮（解析 legacy 值时有保护）。
- [ ] 交节精度 Store（JieQiEntryStrategyStore）默认值为“按分钟/秒”，初始化与持久化逻辑正确。
 - [ ] 交节精度 Store（JieQiEntryStrategyStore）默认值为“按分钟”，初始化与持久化逻辑正确；兼容旧值 `minuteSecond`。
