...
void main(){
  group("create daliuren with shehai(涉害)", ()
  {
    ...
    // List<DiZhi> monthGeneralSeq = DaLiuRenKePan.changeDiZhiSeq(monthGeneral, diZhiList);
    List<DiZhi> monthGeneralSeq = tianPanList_dev;
    ...
    Map<DiZhi,DiZhi> currentTianDiMapper = Map<DiZhi,DiZhi>.fromIterables(diPanList_dev, monthGeneralSeq);
    ...
    test("两个贼 2  --- 涉害 1", () {
      List<DiZhi> diPanList_dev="子丑寅卯辰巳午未申酉戌亥".split("").map((e)=>DiZhi.getFromValue(e)!).toList();
      List<DiZhi> tianPanList_dev="戌亥子丑寅卯辰巳未申酉午".split("").map((e)=>DiZhi.getFromValue(e)!).toList();
    ...
    test("三个贼 一个克 --- 涉害 3 --- 缀",(){
      List<DiZhi> diPanList_dev="子丑寅卯辰巳午未申酉戌亥".split("").map((e)=>DiZhi.getFromValue(e)!).toList();
      List<DiZhi> tianPanList_dev="未申酉戌亥子丑寅卯辰巳午".split("").map((e)=>DiZhi.getFromValue(e)!).toList();
      Map<DiZhi,DiZhi> currentTianDiMapper = Map<DiZhi,DiZhi>.fromIterables(diPanList_dev, tianPanList_dev);
      for (var di in diSeq) {
        currentPanWithGods[di] = DaLiuRenGong(guiRen:currentDiGodsMapper[di]!, skyPanDiZhi: currentTianDiMapper[di]!, groundPanDiZhi: di);
      }
...
FourClass createFourClass(JiaZi dayJiaZi,String first,String second,String third,String fourth,Map<DiZhi,DaLiuRenGong> eachGongMapper) {
  ...
  return FourClass.generate(
      firstGround: TianGan.getFromValue(first
          .split("")
          .last)!,
      firstSky: DiZhi.getFromValue(first
          .split("")
          .first)!,
      secondSky: DiZhi.getFromValue(second
          .split("")
          .first)!,
      secondGround: DiZhi.getFromValue(second
          .split("")
          .last)!,

      thirdSky: DiZhi.getFromValue(third
          .split("")
          .first)!,
      thirdGround: DiZhi.getFromValue(third
          .split("")
          .last)!,

      fourthSky: DiZhi.getFromValue(fourth
          .split("")
          .first)!,
      fourthGround: DiZhi.getFromValue(fourth
          .split("")
          .last)!,
