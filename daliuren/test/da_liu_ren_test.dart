...
void main() {
  group("create daliuren with zeike(贼克)", ()
  {
    ...
    test("戊申月 丁巳日 辛丑 巳将", () {
      ...
      List<DiZhi> diPanSeq = "子丑寅卯辰巳午未申酉戌亥".split("").map((e) => DiZhi.getFromValue(e)!).toList();
      List<DiZhi> tianPanSeq = "辰巳午未申酉戌亥子丑寅卯".split("").map((e) => DiZhi.getFromValue(e)!).toList();
      List<GuiRen> godsSeq =  "龙勾合雀腾贵后阴玄常虎空".split("").map((g)=>GuiRen.getBySingleName(g)).toList();
      List<TianGan> tianGanSeq = "丙丁戊己庚辛壬癸○○甲乙".split("").map((e)=>TianGan.getFromValue(e) ?? TianGan.KONG_WANG).toList();
      Map<DiZhi,DaLiuRenGong> mapper = {};
      for (int i = 0; i < diPanSeq.length; i++){
        mapper[diPanSeq[i]] = DaLiuRenGong(guiRen: godsSeq[i], skyPanDiZhi: tianPanSeq[i], groundPanDiZhi: diPanSeq[i],tianGan:tianGanSeq[i]);
      }
      ...
      // 四课
      FourClass theFour = FourClass.fastGenerate(dayGanZhi: dayJiaZi, eachGongMapper: mapper);
      ...
      ThreeChuan threeChuan = DaLiuRenKePan.calculateThreeChuan(dayJiaZi,theFour,mapper);
