...
class LiuRenRepositoryImplProtobuf implements LiuRenRepository {
  ...
  Future<Either<Failure, RawPan>> getPan(
    ...
    try {
      ...
      if (dataList == null) {
        return Left(NotFoundFailure("Data list not loaded for ${dayNight.name}"));
      }
      ...
      final rawPan = _mapProtobufDataToRawPan(matchedData, dayNight, dayGanZhi, ganShangZhi);
      return Right(rawPan);
    } catch (e, s) {
  ...
  RawPan _mapProtobufDataToRawPan(
      XuanDaLiuRenData data, EnumDayNight dayNight, JiaZi dayGanZhi, DiZhi ganShangZhi) {
    // Map four classes
    ...
    final guiRenType = dayNight == EnumDayNight.day 
        ? GuiRenType.YANG_GUI 
        : GuiRenType.YIN_GUI;
  ...
  RawEachGong _mapGongInfoToRawEachGong(GongInfo gongInfo) {
    final groundPanDiZhi = _parseStringToDiZhi(gongInfo.groundPanDiZhi) ?? DiZhi.ZI;
