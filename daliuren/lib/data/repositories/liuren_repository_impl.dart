...
class LiuRenRepositoryImpl implements LiuRenRepository {
  ...
  @override
  Future<Either<Failure, void>> initializeDatabase(Map<String, List<Map<String, dynamic>>> initialRawData) async {
    try {
      ...
      if (!isInitialized) {
        ...
        final List<JuMappingDataModel> juMappings = (initialRawData['ju_mapper'] ?? [])
            .map((json) => JuMappingDataModel.fromJson(json)) // Assuming json is already Map<String, dynamic>
            .toList();

        final List<YuDingDaLiuRenDataModel> yuDingEntries = (initialRawData['yuding_daliuren'] ?? [])
        ...
        (initialRawData['甲午庚牛羊_阳'] ?? []).forEach((json) {
          ...
          mutableJson['yinYangDun'] = common_enums.YinYang.YANG.name; // Store as string name of enum
          presetPans.add(DaLiuRenPanDataModel.fromJson(mutableJson));
        });
        (initialRawData['甲午庚牛羊_阴'] ?? []).forEach((json) {
          final Map<String, dynamic> mutableJson = Map<String, dynamic>.from(json);
          mutableJson['yinYangDun'] = common_enums.YinYang.YIN.name; // Store as string name of enum
          presetPans.add(DaLiuRenPanDataModel.fromJson(mutableJson));
        });
      ...
      return Left(DatabaseFailure("Database initialization failed: ${e.toString()}", s));
    }
  }
  ...
  Future<Either<Failure, LiuRenPan>> getLiuRenPan(PanInput input) async {
    try {
      ...
      if (input.inputType == PanInputType.GANZHI_INPUT &&
          input.dayJiaZi != null &&
          input.timeJiaZi != null && // timeJiaZi is used to derive ShiChen DiZhi
        ...
        if (presetPanDbo != null) {
          ...
          print("Preset pan not found for Day: ${input.dayJiaZi}, Time Branch: $timeDiZhiName, Dun: ${input.yinYangDun}. Calculation would be needed.");
      ...
      if (input.inputType == PanInputType.TIME_INPUT) {
         print("Calculating LiuRenPan for time: ${input.dateTime} (Placeholder - Calculation Not Implemented)");
         ...
         return Right(LiuRenPan(
            ...
            dayGanZhi: "甲子", timeGanZhi: "甲子", shiChenZhiName: "子",
            yueJiangName: "登明 (Calc)", guiRenType: "阳贵 (Calc)",
            heavenPlate: {}, earthPlate: {},
            fourClasses: [], threeChuans: [],
      ...
      return Left(GenericFailure("LiuRenPan processing for the given input is not fully implemented or no data found."));

    } catch (e, s) {
      print("Repository: Failed to get LiuRenPan: $e \n$s");
      return Left(GenericFailure("Failed to process LiuRenPan request: ${e.toString()}", s));
    }
  }

  /// Fetches a Yu Ding interpretation entry from the local data source and maps it to a domain entity.
  @override
  Future<Either<Failure, YuDingEntry>> getYuDingEntry(String dayJiaZiName, String ganShangDiZhiName) async {
    try {
      final entryDbo = await _localDataSource.getYuDingEntry(dayJiaZiName, ganShangDiZhiName);
      if (entryDbo != null) {
        return Right(_mapYuDingDboToEntity(entryDbo));
      } else {
        // Return a specific failure type if entry is not found.
        return Left(NotFoundFailure("YuDing entry not found for $dayJiaZiName, $ganShangDiZhiName"));
      }
    } catch (e, s) {
      print("Repository: Failed to get YuDingEntry: $e \n$s");
      return Left(DatabaseFailure("Failed to get YuDingEntry: ${e.toString()}", s));
    }
  }
  ...
  LiuRenPan _mapPresetPanDboToEntity(PresetPanEntryDb dbo) {
    ...
    Map<String, LiuRenGongEntity> mapHeavenPlate(Map<String, DaLiuRenGongDataModel> plateDataModel) {
      return plateDataModel.map((key, value) => MapEntry(key, _mapGongDataModelToEntity(value)));
    }
    Map<String, LiuRenGongEntity> mapEarthPlate(Map<String, DaLiuRenGongDataModel> plateDataModel) {
      return plateDataModel.map((key, value) => MapEntry(key, _mapGongDataModelToEntity(value)));
    }
    ...
    List<FourClassKeEntity> mapFourClass(FourClassDataModel fcModel) {
      return [
        _mapEachClassDataModelToKeEntity(fcModel.first, fcModel.firstClassDayGan),
    ...
    domain_nine_zong_men.NineZongMen nineZongMenEnum = domain_nine_zong_men.NineZongMen.values
        .firstWhere((e) => e.name == dbo.nineZongMenName, orElse: () {
            // Log a warning if an unknown NineZongMen name is encountered.
            print("Warning: Unknown NineZongMen name '${dbo.nineZongMenName}' found in DBO. Defaulting to UNKNOWN.");
            return domain_nine_zong_men.NineZongMen.UNKNOWN;
        });

    // Convert string representations of enums from DBO to actual enum types.
    common_enums.JiaZi dayJiaZiEnum = common_enums.JiaZi.fromString(dbo.dayJiaZiName);
    common_enums.DiZhi shiChenEnum = common_enums.DiZhi.fromString(dbo.shiChenName);
    common_enums.YinYang yinYangEnum = common_enums.YinYang.fromString(dbo.yinYangDun);
    ...
    final juNumKeyEntry = common_enums.ConstResourcesMapper.chineseNumberMapper.entries
        .firstWhere((entry) => entry.value == dbo.juNumberName.replaceAll("局", ""),
                    orElse: () => const MapEntry(-1, "")); // Return a dummy entry if not found

    if (juNumKeyEntry.key != -1) { // Check if a valid entry was found
      juNumber = juNumKeyEntry.key;
    }


    return LiuRenPan(
      panDateTime: null, // Preset pans from JSON typically don't have a specific original cast time.
      dayJiaZiName: dbo.dayJiaZiName,
      dayGanZhi: dbo.dayJiaZiName, // Assuming dayJiaZiName is the full GanZhi string.
      ...
      yueJiangName: null, // Preset pans might not explicitly store YueJiang; could be derived if needed.
      guiRenType: dbo.yinYangDun == "yang" ? "阳贵" : "阴贵", // Simplified mapping for GuiRen type.

      heavenPlate: mapHeavenPlate(dbo.heavenPlateJson), // DBO field is Map<String, DaLiuRenGongDataModel> due to TypeConverter
      ...
      keTiComplement: [dbo.nineZongMenName], // Initially use the raw name as a complement; can be expanded.
      sourceDescription: "Loaded from Preset ${dbo.yinYangDun.toUpperCase()} Pan",
  ...
  FourClassKeEntity _mapEachClassDataModelToKeEntity(EachClassDataModel model, [common_enums.TianGan? dayGan]) {
      return FourClassKeEntity(
          order: model.order,
          sky: model.sky,
          ground: model.ground,
          guiRen: model.guiRen,
          dayGan: model.isFirstClass ? dayGan : null, // Day Gan is only relevant for the first Ke.
      );
  }

  /// Maps [EachChuanDataModel] (DTO) to [ThreeChuanChuanEntity] (Domain Entity).
  ThreeChuanChuanEntity _mapEachChuanDataModelToChuanEntity(EachChuanDataModel model) {
  ...
  YuDingEntry _mapYuDingDboToEntity(YuDingEntryDb dbo) {
    ...
    title += "${common_enums.ConstResourcesMapper.chineseNumberMapper[dbo.juNumber] ?? dbo.juNumber.toString()}局 ";
    title += "干上${dbo.juName}";

    return YuDingEntry(
      title: title,
      原文: dbo.bodyJson.toList(), // Set<String> from DBO's TypeConverter converted to List<String>
      課義: dbo.meaning,
      解曰: dbo.explain,
      斷曰: dbo.predication,
      杂占: dbo.detailsJson, // Map<String, String> from DBO's TypeConverter
      经典: dbo.booksJson,   // Map<String, String> from DBO's TypeConverter
    );
  }
}
...
class NotFoundFailure extends Failure {
  NotFoundFailure(String message, [StackTrace? stackTrace]) : super(message, stackTrace);
}
