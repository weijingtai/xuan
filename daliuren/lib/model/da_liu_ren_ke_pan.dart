...
import 'package:daliuren/model/three_chuan_bi_yong.dart';
...
class DaLiuRenKePan extends DaLiuRenPanel {
  ...
  static final TWELVE_GODS_LIST = ["贵人","腾蛇","朱雀","六合","勾陈","青龙","天空","白虎","太常","玄武","太阴","天后"];
  ...
  static final List<DiZhi> DAY_CHEN = [DiZhi.MAO,DiZhi.CHEN,DiZhi.SI,DiZhi.WU,DiZhi.WEI,DiZhi.SHEN]; // 白天的时辰
  ...
  DaLiuRenKePan({
    ...
    List<DiZhi> monthGeneralSeq = changeDiZhiSeq(monthGeneral.generalZhi, diZhiList);
    ...
    // 计算贵人位
    Tuple2<bool,DiZhi> guiRenResult = calculateGuiRenLocationWithFirst(dayJiaZi,timeJiaZi);
    isDayGuiRen = guiRenResult.item1;
    guiRenDiZhi = guiRenResult.item2;

    _godsMapper = calculateGodsMapper(timeJiaZi.diZhi,_tianDiPanMapper,guiRenDiZhi);
    // 根据日柱，计算天盘的干支
    ...
    // 将时旬根据地支为Key 转为Map
    Map<DiZhi,JiaZi> _tmpMapper = {};
    currentJiaZiXunList.forEach((jiaZi){
      _tmpMapper[jiaZi.diZhi] = jiaZi;
    });
    DiZhi.listAll.forEach((diZhi){
      ...
      gongMapper[diZhi] = DaLiuRenGong(
          ...
          tianGan: _tmpMapper[skyDiZhi]?.tianGan,
          jiaZi: _tmpMapper[diZhi]);
    });
  ...
  static ThreeChuan calculateThreeChuan(JiaZi dayJiaZi,FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper){

    // 首先进行 伏吟 与 反吟
    // 伏吟
     ThreeChuan? checkFuYinFirstResult = checkByFuYin(dayJiaZi,fourClass,gongMapper);
     if (checkFuYinFirstResult != null){
       return checkFuYinFirstResult;
     }
     // 反吟
     ThreeChuan? checkFanYinFirstResult = checkByFanYin(dayJiaZi,fourClass,gongMapper);
     if (checkFanYinFirstResult != null){
       return checkFanYinFirstResult;
     }

      // 八专 -- 两课
     ThreeChuan? checkBaZhuanFirstResult = checkByBaZhuan(dayJiaZi,fourClass,gongMapper);
     ...
     ThreeChuan? checkMaoXingResult = checkByMaoXing(dayJiaZi, fourClass, gongMapper);
  ...
  static ThreeChuan? checkByBieZe(JiaZi dayJiaZi,FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper){
    // 是否为四课齐备
    ...
    if (fourClass.listAllClass.any((e)=>e.zeiKeType != null)
      ...
      ThreeChuan? threeChuan = checkByZeiKe(dayJiaZi, fourClass, gongMapper,callByBieZe: true);
      if (threeChuan != null){
        return threeChuan;
      }
    ...
    if (dayJiaZi.tianGan.isYang){
      // 取天干五合中另一干
      Tuple2<TianGanFiveCombine,TianGan> combine = TianGanFiveCombine.getFiveCombineWithOtherTianGan(gan);
      ...
      List<String> sanHeStrList = DiZhiSanHe.getBySingleDiZhi(dayDiZhi)!.name.split("").toList();
      ...
      chuChuanGong = gongMapper.values.firstWhere((e)=>e.skyPanDiZhi == firstDiZhi);
    }
    DaLiuRenGong firstGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == chuChuanGong.skyPanDiZhi);
    DaLiuRenGong secondGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == dayGanSkyDiZhi);
    DaLiuRenGong thirdGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == dayGanSkyDiZhi);
    return _buildThreeChuan(dayJiaZi,NineZongMen.BIE_ZE, firstGong, secondGong, thirdGong);
  }

  // 伏吟
  static ThreeChuan? checkByFuYin(JiaZi dayJiaZi,FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper){
    // 伏吟的规则为 “寅上寅”，“申上申”
    ...
    if (fourClass.first.zeiKeType!=null){
      ...
      // 【自任格】
      // 无克伏吟， 阳日取干上神，阴日取支上神
      firstChuanDiZhi = dayJiaZi.tianGan.isYang ? fourClass.first.sky : fourClass.third.sky;
      // 初传为阴日 三传有无刑都为【自信格】

      //初传或中传 有自刑，兼【杜传格】
      if (firstChuanDiZhi.isSelfXing){
        // 如果初传为自刑，阴日则取干上神，阳日取支上神，为中传，若中传也自刑，则取中传所冲的地支为末传
        secondChuanDiZhi = dayJiaZi.tianGan.isYang ? fourClass.third.sky :fourClass.first.sky;
        if (secondChuanDiZhi.isSelfXing){
          thirdChuanDiZhi = DiZhiChong.getOtherDiZhi(secondChuanDiZhi);
        }else{
          thirdChuanDiZhi = DiZhiXing.getOtherDiZhi(secondChuanDiZhi);
      ...
      if (dayJiaZi.tianGan.isYin && [JiaZi.DING_MAO,JiaZi.JI_MAO,JiaZi.XIN_MAO].contains(dayJiaZi)) {
    ...
    DaLiuRenGong firstGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == firstChuanDiZhi);
    DaLiuRenGong secondGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == secondChuanDiZhi);
    DaLiuRenGong thirdGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == thirdChuanDiZhi);
    return _buildThreeChuan(dayJiaZi,NineZongMen.FU_YIN, firstGong, secondGong, thirdGong);
    // return ThreeChuan(
  ...
  // 反吟
  static ThreeChuan? checkByFanYin(JiaZi dayJiaZi,FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper){
    ...
    if ([JiaZi.WU_CHEN,JiaZi.WU_XU].contains(dayJiaZi)){
      if (fourClass.first.sky == DiZhi.HAI && [DiZhi.XU,DiZhi.CHEN].contains(fourClass.third.sky)){
        DaLiuRenGong firstGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi ==  DiZhi.SI);
        DaLiuRenGong secondGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == DiZhi.HAI);
        DaLiuRenGong thirdGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi ==  DiZhi.SI);

        return _buildThreeChuan(dayJiaZi,NineZongMen.FAN_YIN, firstGong, secondGong, thirdGong);
      }
    ...
    if (fourClass.listAllClass.any((e)=>e.zeiKeType != null)){
      ...
      DiZhi dayZhiMengSheng = DiZhiSanHe.getBySingleDiZhi(dayJiaZi.diZhi)!.content.first;
  ...
  static ThreeChuan? _buildThreeChuan(JiaZi dayJiaZi,NineZongMen nineZongMen,DaLiuRenGong firstGong,DaLiuRenGong secondGong,DaLiuRenGong thirdGong){

    EachChuan firstChuan = EachChuan(
        order: 1,
        guiRen: firstGong.guiRen,
        liuQin: LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.gan, firstGong.skyPanDiZhi),
        diZhi: firstGong.skyPanDiZhi);
    EachChuan secondChuan = EachChuan(
        order: 2,
        guiRen: secondGong.guiRen,
        liuQin: LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.gan, secondGong.skyPanDiZhi),
        diZhi: secondGong.skyPanDiZhi);
    EachChuan thirdChuan = EachChuan(
        order: 3,
        guiRen: thirdGong.guiRen,
        liuQin: LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.gan, thirdGong.skyPanDiZhi),
        diZhi: thirdGong.skyPanDiZhi);
    return ThreeChuan(nineZongMen: nineZongMen, first: firstChuan, second: secondChuan, third: thirdChuan);

  }
  static ThreeChuan? checkByBaZhuan(JiaZi dayJiaZi,FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper){
    ...
    // 2.判断是否为八专日
    if(![JiaZi.JIA_YIN,JiaZi.GENG_SHEN,JiaZi.GUI_CHOU,JiaZi.DING_WEI,JiaZi.JI_WEI].contains(dayJiaZi)){
      return null;
    }
    ...
    if (dayJiaZi.gan.isYang){
      // 以日干上神在天盘顺时针数3神为初传；
      DiZhi dayGanSkyDiZhi = fourClass.first.sky;
      List<DiZhi> clockwiseDiZhiList = gongMapper.values.map((e)=>e.skyPanDiZhi).toList();
      ...
      firstGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == chuChuanDiZhi);
      ...
      List<DiZhi> clockwiseDiZhiList = gongMapper.values.map((g)=>g.skyPanDiZhi).toList();
    ...
    // 中传末传都用 日干上神
    DiZhi dayGanSkyDiZhi = fourClass.first.sky;
    DaLiuRenGong gong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == dayGanSkyDiZhi);
    ...
    return _buildThreeChuan(dayJiaZi,NineZongMen.BA_ZHUAN, firstGong, gong, gong);

  }
  static ThreeChuan? checkByZeiKe(JiaZi dayJiaZi,FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper,{bool callByBieZe = false}){
    ...
    fourClass.listAllClass.forEach((each){
    ...
    if (zeiIndexList.isNotEmpty){
      ...
      if (zeiIndexList.length == 1){
        ...
        if (zeiIndexList.length == 2){
          if(zeiIndexList.map((i)=> fourClass.getAsIndex(i)).toList().map((e)=>"${e.ground.value}${e.sky.value}").toSet().length == 1){
            ...
            // print("重审课（一个贼，一个克，取贼不取克）");
            chuChuanZhu = fourClass.getAsIndex(zeiKeIndexAt);
            Tuple3<EachChuan,EachChuan,EachChuan> chuanTuple3 = createEachThreeChuan(dayJiaZi, chuChuanZhu, gongMapper);
            EachChuan firstChuan = chuanTuple3.item1;
            EachChuan secondChuan = chuanTuple3.item2;
            EachChuan thirdChuan = chuanTuple3.item3;
            return ThreeChuanZeiKe(type: zeiKeType!,zeiKeType: chuChuanZhu.zeiKeType!, first: firstChuan, second: secondChuan, third: thirdChuan);
          }
        }
        if (fourClass.isThreeClassOnly){
          // print("只备三课");
          ...
          if (fourClass.first.otherSameSkyGroundIndexList != null){
            duplicateClass = fourClass.getAsIndex(fourClass.first.otherSameSkyGroundIndexList!.first);
          }else{
            duplicateClass = fourClass.listAllClass.firstWhere((t)=>t.otherSameSkyGroundIndexList!=null);
          }
        ...
        if (sameYinYangWithDayGan.isEmpty){
          ...
          Tuple2<SheHaiType,EachClass>? result = sheHai(dayJiaZi,fourClass,zeiIndexList.map((i)=>fourClass.getAsIndex(i)).toList(),gongMapper,false);
          if (result != null){
          ...
          if (callByBieZe){
            Set<String> _tmpSet = sameYinYangWithDayGan.map((e)=>"${e.sky}${e.ground}").toSet();
            ...
            if (_tmpSet.length == 1){
              ...
              if (sameYinYangWithDayGan.length == 3){
                ...
                List<EachClass> _newSameYinYangWithDayGan = [];
                sameYinYangWithDayGan.forEach((e){
                  if (e.otherSameSkyGroundIndexList!=null){
                    if (e.order != shouldRemovedOrder){
                      _newSameYinYangWithDayGan.add(e);
                ...
                sameYinYangWithDayGan = _newSameYinYangWithDayGan;
              }
              // print("不是别责，按照正常的进行涉害");
              // 不是别责，按照正常的进行涉害
              nineZongMenType = NineZongMen.SHE_HAI;
              Tuple2<SheHaiType,EachClass>? result = sheHai(dayJiaZi,fourClass,sameYinYangWithDayGan,gongMapper,false);
              if (result != null){
                sheHaiType = result.item1;
                chuChuanZhu = result.item2;
              }else{
      ...
      if (keIndexList.length == 1){
        ...
        if (fourClass.isThreeClassOnly){
          // 只被三课，需要将重复的课移除
          EachClass duplicateClass = fourClass.listAllClass.firstWhere((t)=>t.otherSameSkyGroundIndexList!=null);
          for (var i in duplicateClass.otherSameSkyGroundIndexList!){
        ...
        if (sameYinYangWithDayGan.isEmpty){
          ...
          // 不是别责，按照正常的进行涉害
          nineZongMenType = NineZongMen.SHE_HAI;
          Tuple2<SheHaiType,EachClass>? result = sheHai(dayJiaZi,fourClass,keIndexList.map((i)=>fourClass.getAsIndex(i)).toList(),gongMapper,true);
          if (result != null){
            sheHaiType = result.item1;
            chuChuanZhu = result.item2;
          }else{
          ...
          if (callByBieZe){
            ...
            // 不是别责，按照正常的进行涉害
            nineZongMenType = NineZongMen.SHE_HAI;
            Tuple2<SheHaiType,EachClass>? result = sheHai(dayJiaZi,fourClass,sameYinYangWithDayGan,gongMapper,true);
            if (result != null){
              sheHaiType = result.item1;
              chuChuanZhu = result.item2;
            }else{
    ...
    if (nineZongMenType == NineZongMen.BI_YONG){
      return ThreeChuan(nineZongMen: nineZongMenType!, first: firstChuan, second: secondChuan, third: thirdChuan);
    }
    else if (nineZongMenType == NineZongMen.SHE_HAI){
      return ThreeChuanSheHai(type: sheHaiType!, first: firstChuan, second: secondChuan, third: thirdChuan);
    }
  ...
  @deprecated
  static ThreeChuan? checkByZeiKe2(JiaZi dayJiaZi, FourClass fourClass, Map<DiZhi, DaLiuRenGong> gongMapper) {
    List<int> keIndexList = []; // 上克下 为顺克，保存一到四课的index,如何存在“克”则将index此List
    ...
    if (zeiIndexList.isNotEmpty) {
      ...
      if (zeiIndexList.length == 1) {
        zeiKeIndexAt = zeiIndexList.first;
        nineZongMenType = NineZongMen.ZEI_KE;
        zeiKeType = keIndexList.isEmpty ? ZeiKeType.SHI_RU_KE : ZeiKeType.CHONG_SHEN_KE;
        chuChuanZhu = fourClass.getAsIndex(zeiKeIndexAt);
      }
      else {
...
// 处理多个贼课或克课
  @deprecated
  static EachClass? _handleMultipleZeiKe(JiaZi dayJiaZi, FourClass fourClass, Map<DiZhi, DaLiuRenGong> gongMapper, List<int> indexList, bool isKe) {
    List<EachClass> sameYinYangWithDayGan = [];
    ...
    if (sameYinYangWithDayGan.isEmpty) {
      ...
      Tuple2<SheHaiType, EachClass>? result = sheHai(dayJiaZi, fourClass, sameYinYangWithDayGan, gongMapper, isKe);
      return result?.item2;
    }
  }
  static ThreeChuan? checkByYaoKe(JiaZi dayJiaZi, FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper,{bool callByBieZe = false}){
    // 遥克 1：上神克日干
    YaoKeType? yaoKeType;
    EachClass? chuChuanZhu;
    ...
    if (yaoKeSkyKeDayGanList.isNotEmpty){
      ...
      if (yaoKeSkyKeDayGanList.length == 1){
        ...
        if (callByBieZe && yaoKeSkyKeDayGanList.length == 2){
          ...
          Tuple3<EachChuan,EachChuan,EachChuan> chuanTuple3 = createEachThreeChuan(dayJiaZi, yaoKeSkyKeDayGanList.first, gongMapper);
          EachChuan firstChuan = chuanTuple3.item1;
          EachChuan secondChuan = chuanTuple3.item2;
          EachChuan thirdChuan = chuanTuple3.item3;
        ...
        // 如果还有多个则“涉害”
        List<EachClass> sameYinYangWithDayGan = [];
        yaoKeSkyKeDayGanList.forEach((each){
          if (each.isSkySameYinYangWithDayGan){
            // 只保留与与日干同阴阳的
            sameYinYangWithDayGan.add(each);
          }
      ...
      if (yaoKeDayGanKeSkyList.isNotEmpty){
        ...
        if (yaoKeDayGanKeSkyList.length == 1){
          ...
          // 如果还有多个则“涉害”
          List<EachClass> sameYinYangWithDayGan = [];
          yaoKeDayGanKeSkyList.forEach((each){
            if (each.isSkySameYinYangWithDayGan){
              // 只保留与与日干同阴阳的
              sameYinYangWithDayGan.add(each);
            }
  ...
  // 昴星，前提四课全
  static ThreeChuan? checkByMaoXing(JiaZi dayJiaZi, FourClass fourClass,Map<DiZhi,DaLiuRenGong> gongMapper){
    ...
    if (dayJiaZi.tianGan.yinYang == YinYang.YANG){
      ...
      first = EachChuan(
        ...
        liuQin:LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.tianGan, chuChuanGong.skyPanDiZhi),
      ...
      // 阳日干 上神为末传
      thirdChuanGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == fourClass.first.sky);
    }
    else{
      // 阴日干取酉下发初传
      DaLiuRenGong underYouFaYongGOng = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == DiZhi.YOU)!;
      chuChuanDiZhi = underYouFaYongGOng.groundPanDiZhi;
      chuChuanGong = gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == chuChuanDiZhi);
      ...
      // 阳日干 干上神为中传
      secondChuanGong =  gongMapper.values.firstWhere((g)=>g.skyPanDiZhi == fourClass.first.sky)!;
      // 阳日干 支上神为末传
    ...
    var second = EachChuan(
      ...
      liuQin:LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.tianGan, secondChuanGong.skyPanDiZhi),
    ...
    var third = EachChuan(
      ...
      liuQin:LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.tianGan, thirdChuanGong.skyPanDiZhi),
      tianGan: thirdChuanGong.tianGan,
    );
    return ThreeChuan(nineZongMen:NineZongMen.MAO_XING,first: first, second: second, third: third);
  ...
  static Tuple3<EachChuan,EachChuan,EachChuan> createEachThreeChuan(JiaZi dayJiaZi, EachClass chuChuanZhu, Map<DiZhi,DaLiuRenGong> gongMapper){
    DaLiuRenGong firstChuanGong = gongMapper.values.firstWhere((gong)=>gong.skyPanDiZhi == chuChuanZhu!.sky);
    EachChuan firstChuan = EachChuan(
      order:1,
      diZhi:firstChuanGong.skyPanDiZhi,
      guiRen:firstChuanGong.guiRen,
      liuQin:LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.gan, firstChuanGong.skyPanDiZhi),
    ...
    EachChuan secondChuan = EachChuan(
      ...
      diZhi:secondChuanGong.skyPanDiZhi,
      guiRen:secondChuanGong.guiRen,
      liuQin:LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.gan, secondChuanGong.skyPanDiZhi),
        tianGan: secondChuanGong.tianGan,
    );
    ...
    EachChuan thirdChuan = EachChuan(
      ...
      diZhi:thirdChuanGong.skyPanDiZhi,
      guiRen:thirdChuanGong.guiRen,
      liuQin:LiuQin.getLiuQinByForTianGanDiZhi(dayJiaZi.gan, thirdChuanGong.skyPanDiZhi),
        tianGan: thirdChuanGong.tianGan,
    );
    return Tuple3<EachChuan,EachChuan,EachChuan>(firstChuan,secondChuan,thirdChuan);
  }

  static Tuple2<SheHaiType,EachClass> sheHaiZhui(JiaZi dayGanZhi,FourClass fourClass){
  ...
  static Tuple2<SheHaiType,EachClass>? sheHai(
    ...
    EachClass maxSheHai = allEachClass.reduce((a,b) => a.sheHaiTimes!>b.sheHaiTimes!?a:b);
    // print(isKe?"涉害 克":"涉害 贼");
    return Tuple2(SheHaiType.SHEN_QIAN, maxSheHai);
  }

  // TODO: remove `Map<DiZhi, DiZhi> currentPanel` from arguments
  static int sheHaiShenQianTimes(DiZhi sky,Map<DiZhi,DaLiuRenGong> gongMapper,bool isKe){
    // 将地盘与天盘分为两个独立的list
    ...
    for (int i =0; i<newDiPanList.length ; i++){
      ...
      if (isKe){
        // 统计克宫的次数
        if (FiveXingRelationship.checkRelationship(currentDiPanGongZhi.fiveXing, skyFiveXing)==FiveXingRelationship.KE){
          sheHaiCounter += 1;
          // print("宫${currentDiPanGongZhi.value} 克");
        }
        // 统计克“寄宫天干”次数
        List<TianGan>? jiGanList = reversedTenGanJiGongMapper[currentDiPanGongZhi];
        if (jiGanList!=null){
          // 当前地盘宫位存在寄宫天干，一一进行五行生克比对
          jiGanList.forEach((e){
            if(FiveXingRelationship.checkRelationship(e.fiveXing, skyFiveXing)==FiveXingRelationship.KE){
              sheHaiCounter += 1;
        ...
        DiZhi currentDiPanGongZhi = newDiPanList[i];
        // 统计克宫的次数
        if (FiveXingRelationship.checkRelationship(skyFiveXing,currentDiPanGongZhi.fiveXing)==FiveXingRelationship.KE){
          sheHaiCounter += 1;
          // print("宫${currentDiPanGongZhi.value} 克");
        }
        // 统计克“寄宫天干”次数
        ...
        if (jiGanList!=null){
          ...
          for (var e in jiGanList) {
            if(FiveXingRelationship.checkRelationship(skyFiveXing,e.fiveXing)==FiveXingRelationship.KE){
              sheHaiCounter += 1;
              // print("宫${currentDiPanGongZhi.value} 寄干:${e.value} 克");
            }
          }
  ...
  static Tuple2<SheHaiType,EachClass>?  sheHaiSameTimesSelectFromList(
    ...
    switch(counter){
      case 1:
        _list = [DiZhi.YIN,DiZhi.SHEN,DiZhi.SI,DiZhi.HAI];
        break;
      case 2:
        _list =  [DiZhi.ZI,DiZhi.WU,DiZhi.MAO,DiZhi.YOU];
        break;
      case 3:
      // 涉害中不取四季
        _list = [DiZhi.CHEN,DiZhi.XU,DiZhi.CHOU,DiZhi.WEI];
        return null;
        break;
      default:
    ...
    if (eachClassList.length == 2){
      ...
      bool firstIn = _list.contains(first.ground);
      bool secondIn = _list.contains(second.ground);
      if (firstIn && secondIn){

        return null;
      }else if(firstIn && !secondIn){
        return Tuple2(counter==1?SheHaiType.JIAN_JI:SheHaiType.CHA_WEI,first);
      }else if(!firstIn && secondIn){
        return Tuple2(counter==1?SheHaiType.JIAN_JI:SheHaiType.CHA_WEI,second);
      }else{
        // 没仲取季
        counter += 1;
      ...
      bool firstIn = _list.contains(first.ground);
      bool secondIn = _list.contains(second.ground);
      bool thirdIn = _list.contains(third.ground);
      ...
      if (newCheckList.length == 1){
        ...
        return Tuple2(counter==1?SheHaiType.JIAN_JI:SheHaiType.CHA_WEI,newCheckList[0]);
      ...
      bool secondIn = _list.contains(second.ground);
      bool thirdIn = _list.contains(third.ground);
      bool fourthIn = _list.contains(fourth.ground);
      if (firstIn && secondIn && thirdIn){
        return null;
      }
      List<EachClass> newCheckList = [];
  ...
  static Map<DiZhi,GuiRen> calculateGodsMapper(DiZhi timeDiZhi,Map<DiZhi,DiZhi> tianDiPanMapper,DiZhi guiRenDiZhi){
    /// @return: key 是 地盘地支
    ...
    bool isAntiClockedGodsSeq = [DiZhi.SI,DiZhi.WU,DiZhi.WEI,DiZhi.SHEN,DiZhi.YOU,DiZhi.XU].contains(diPanLuoGong);
    if(isAntiClockedGodsSeq){
      ...
      return Map<DiZhi,GuiRen>.fromIterables(diSeq4Gods, GuiRen.antiClockwiseList);
    }else{
      return Map<DiZhi,GuiRen>.fromIterables(diSeq4Gods,  GuiRen.clockwiseList);
    }
  }
  static FourClass calculateFourClass(JiaZi dayGanZhi,Map<DiZhi,DaLiuRenGong> eachGongMapper){
    return FourClass.fastGenerate(dayGanZhi: dayGanZhi, eachGongMapper: eachGongMapper);
  }

  static List<DiZhi> changeDiZhiSeq(DiZhi start,List<DiZhi> originalSeq,{bool isReversed =false}){
    ...
    // print(timeZhiIndex);
    List<DiZhi> newDiZhiList = oldList.sublist(timeZhiIndex).toList(growable: true);
    // print(newDiZhiList);
   ...
   */
  @deprecated
  static Tuple2<bool,DiZhi> calculate_gui_ren(JiaZi dayGanZhi,JiaZi timeGanZhi){
    bool isDay = DAY_CHEN.contains(timeGanZhi.diZhi);
    var locationDiZhi = isDay?dayNightGuiRenMapper[dayGanZhi.tianGan]!.item1:dayNightGuiRenMapper[dayGanZhi.tianGan]!.item2;
    return Tuple2(isDay, locationDiZhi);
  }
  static Tuple2<bool,DiZhi> calculateGuiRenLocationWithFirst(JiaZi dayGanZhi,JiaZi timeGanZhi){
    /// 根据 “甲戊庚牛羊”进行计算
