...
class FourClass{
  ...
  FourClass.fastGenerate({
    ...
    first = FirstClass(sky: firstGong.skyPanDiZhi, ground: jiZhi, guiRen: firstGong.guiRen, tianGan: dayGan);
    ...
    second = createEachClass(dayGan,1, secondGong.skyPanDiZhi, secondGround, secondGong.guiRen);

    var thirdGround = dayZhi;
    var thridGong = eachGongMapper[thirdGround]!;
    third = createEachClass(dayGan,2, thridGong.skyPanDiZhi, thirdGround, thridGong.guiRen);

    var fourthGround = thridGong.skyPanDiZhi;
    var fourthGong = eachGongMapper[fourthGround]!;
    fourth = createEachClass(dayGan,3, fourthGong.skyPanDiZhi, fourthGround, fourthGong.guiRen);

    List<EachClass> _classList = [first,second,third,fourth];
    // 检查是否为伏吟
    isFuYin = _checkIsFuYin(_classList);
    isFanYin = _checkIsFanYin(_classList);
    ...
    if (isFuYin || isFanYin){
      ...
      // 只在不是伏吟是查看是否为四课补全
      isFullClass = checkIsFullClass(_classList);
      if (!isFullClass){
        isThreeClassOnly = _setupBieZe(_classList);
      }else{
        isThreeClassOnly = false;
      }
    }
  ...
  FourClass.generate({
    ...
    var firstGods=  eachGongMapper.values.firstWhere((g)=>g.skyPanDiZhi == firstSky).guiRen;
    first = FirstClass(sky: firstSky, ground: jiZhi, guiRen: firstGods, tianGan: firstGround);

    // var secondGround =firstSky;
    // var secondSky = eachGongMapper[secondGround]!.skyPanDiZhi;
    var secondGods=  eachGongMapper.values.firstWhere((g)=>g.skyPanDiZhi == secondSky).guiRen;
    second = createEachClass(dayGan,1, secondSky, secondGround, secondGods);

    // var thirdGround =dayZhi;
    // var thirdSky = eachGongMapper[thirdGround]!.skyPanDiZhi;
    var thirdGods=  eachGongMapper.values.firstWhere((g)=>g.skyPanDiZhi == thirdSky).guiRen;
    ...
    var fourthGods=  eachGongMapper.values.firstWhere((g)=>g.skyPanDiZhi == fourthSky).guiRen;
  ...
  static bool _checkIsFuYin(List<EachClass> classList){
    ...
    return classList[1].sky == classList[1].ground && classList[2].sky == classList[2].ground;
  }
  // 检查是否为伏吟
  static bool _checkIsFanYin(List<EachClass> classList){
    // 1. 检查第二课 与 第三课 是否 为六冲
    return DiZhiChong.getOtherDiZhi(classList[1].sky) == classList[1].ground && DiZhiChong.getOtherDiZhi(classList[2].sky) == classList[2].ground;
  }
  ...
  static bool _setupBieZe(List<EachClass> classList){
    ...
    Map<String,List<int>> _singleSet = Map<String,List<int>>.fromEntries(classList.map((e)=>MapEntry("${e.sky.value}${e.ground.value}",[])));

    classList.forEach((each){
      String _tmpKey = "${each.sky.value}${each.ground.value}";
      _singleSet[_tmpKey]!.add(each.order);
    });
    ...
    if (_singleSet.keys.length == 1){
      classList.forEach((each){
        each.otherSameSkyGroundIndexList = _singleSet.values.first.where((i)=>i!=each.order).toList();
      });
      return false;
    }
    else if (_singleSet.keys.length == 2){
      // 找到 value.length 为 2 的key

      var _tmpKey = _singleSet.keys.firstWhere((key)=>_singleSet[key]!.length == 2);
      // 从 _classList 中找到所有相同"sky""ground"的eachClass
      classList.forEach((each){
        if (_tmpKey == "${each.sky.value}${each.ground.value}"){
          each.otherSameSkyGroundIndexList = _singleSet[_tmpKey]!.where((i)=>i!=each.order).toList();
        }
      });
      return true;
    }
    else if (_singleSet.keys.length == 3){
      // 找到 value.length 为 2 的key
      var _tmpKey = _singleSet.keys.firstWhere((key)=>_singleSet[key]!.length == 2);
      // 从 _classList 中找到所有相同"sky""ground"的eachClass
      classList.forEach((each){
  ...
  static EachClass createEachClass(TianGan dayGan,int order, DiZhi sky, DiZhi ground, GuiRen guiRen){
    var skyKeDayGan = FiveXingRelationship.checkRelationship(dayGan.fiveXing, sky.fiveXing) == FiveXingRelationship.KE;
    var dayGanKeSky = FiveXingRelationship.checkRelationship(sky.fiveXing, dayGan.fiveXing) == FiveXingRelationship.KE;
    bool? isSkyKeDayGan;
    if (skyKeDayGan){
      isSkyKeDayGan = true;
    }else if (dayGanKeSky){
  ...
  factory FourClass.fromJson(Map<String, dynamic> json) => _$FourClassFromJson(json);
