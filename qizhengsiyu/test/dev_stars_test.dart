...
void main(){
  ...
  group("star adjust star:3",(){
    ...
    test('around star 3 same angle 0,0,1', () {
      ...
      expect(resolvedStars.orderedStars.map((r)=>r.angle).toSet(),equals({358.0,2.0,6.0}));
    ...
    test('around star 3 same angle 89,90,91', () {
      ...
      expect(resolvedStars.orderedStars.map((r)=>r.angle).toSet(),equals({86.0,90.0,94.0}));
    ...
    test('around star 3 same angle 359,0,1', () {
      ...
      expect(resolvedStars.orderedStars.map((r)=>r.angle).toSet(),equals({356.0,0.0,4.0}));
    });

    test('around star 6, there are 5 stars in range and other 1 star not in, but when those 5 stars adjusted, The other in range , [90,91,92,93,94, 102]', (){
       ///              [84,88,92,96,100, 102]
      ...
      expect(result.orderedStars.map((s)=>s.angle).toSet(), equals({84.0,88.0,92.0,96.0,100.0}));
    });
  });
  ...
  group("star ",(){
    test("",(){
      ...
      print(StarsResolver.sortCircularAngles([358,350,0,4,359,1,2,5,7,8,9]));
  ...
  group("doResolveSameAngleStars", (){
    ...
    test("doResolveSameAngleStars 2", (){
      UIConstellationModel constellationModel = StarsResolver.doResolveSameAngleStars([
        UIStarModel(
          star: EnumStars.Sun,
          priority: 4,
          originalAngle: 90,
      ...
      Iterable<double> angleList = constellationModel.orderedStars.map((e)=>e.angle);
  ...
  group("sortInRangeStarsNoneSameAngleWithPriority", (){
    test("2 priority=4,3",(){
      UIConstellationModel constellationModel = StarsResolver.sortInRangeStarsNoneSameAngleWithPriority([
        UIStarModel(
          star: EnumStars.Sun,
          priority: 4,
          originalAngle: 91,
      ...
      expect(constellationModel.orderedStars.map((s)=>s.angle), equals([90 ,91]));
    });
    test("3 priority=4,3,2 - [89,90,※91]",(){
      ...
      expect(constellationModel.orderedStars.map((s)=>s.angle), equals([89,90 ,91]));
    });
    test("3 priority=4,3,2 - [89,※91,92]",(){
      ...
      // Iterable<double> angleList = constellationModel.map((e)=>e.angle);
      expect(constellationModel.orderedStars.length, equals(3));
      expect(constellationModel.orderedStars.map((s)=>s.angle), equals([89 ,91,92]));
    });
    ...
    test("3 priority=2,2,1 ",(){
      ...
      // Iterable<double> angleList = constellationModel.map((e)=>e.angle);
      expect(constellationModel.orderedStars.length, equals(3));
      expect(constellationModel.orderedStars.map((s)=>s.angle), equals([90 ,91,92]));
    });
    test("4 priority=2,2,2,1 ",(){
      ...
      expect(constellationModel.orderedStars.map((s)=>s.angle), equals([89,90 ,91,92]));
    });
  ...
  group("sortStarByWithCircularAngle", (){
    test("sortStarByWithCircularAngle", (){
      ...
      final res = StarsResolver.sortStarWithCircularAngleByCenterStar(stars,stars.first).orderedStars;
      expect(res.length, equals(3));
      expect(res[0].star, equals(EnumStars.Sun));
      expect(res[1].star, equals(EnumStars.Moon));
      expect(res[2].star, equals(EnumStars.Golden));
    });
    test("sortStarByWithCircularAngle", (){
      final stars = [
        UIStarModel(
  ...
  group('sortMultipleInRangeStars Tests', () {
    ...
    test('Odd number of angles with one star per angle 3', () {
      ...
      expect(result.orderedStars.map((s)=>s.angle), equals({7.0,11.0,15.0}));
    });
    ...
    test('Odd number of angles , center angle with two stars', () {
      ...
      expect(result.orderedStars.map((s)=>s.angle), equals({5.0,9.0,13.0,17.0}));
    });
    ...
    // 测试偶数个不同角度且每个角度只有一个星体的情况
    test('Even number angles, there a two same angle can be center, and both of them has two stars', () {
      ...
      expect(result.orderedStars.isNotEmpty, true);
      expect(result.orderedStars.length, equals(4));
      expect(result.orderedStars.map((s)=>s.angle).toSet(), equals({4.5,8.5,12.5,16.5}));
    });

    // 测试偶数个不同角度且每个角度只有一个星体的情况
    test('Even number angles, there a two same angle can be center, and both of them has two stars and both side has one star', () {
      // 当有偶数个角度，且可以作为起始中间点的角度上一个有两个星体，另一个也有两个星体'
      Map<double,List<UIStarModel>> starsMapper = {};
      [
        UIStarModel(
      ...
      expect(result.orderedStars.map((s)=>s.angle).toSet(), equals({0.5,4.5,8.5,12.5,16.5,20.5}));
    });
    ...
    test('Odd number angles,center angle with two stars', () {
      ...
      expect(result.orderedStars.isNotEmpty, true);
      expect(result.orderedStars.length, equals(4));
      expect(result.orderedStars.map((s)=>s.angle).toSet(), equals({5.0,9.0,13.0,17.0}));
    });
    ...
    test('Odd number angles,center angle with two stars', () {
      ...
      expect(result.orderedStars.isNotEmpty, true);
      expect(result.orderedStars.length, equals(5));
      expect(result.orderedStars.map((s)=>s.angle).toSet(), equals({3.0,7.0,11.0,15.0,19.0}));
      expect(result.orderedStars[2].priority, equals(4));
    });

    // 测试奇数个不同角度且每个角度只有一个星体的情况
    test('Odd number angles,center angle with two stars, one side angle has two stars', () {
      // 当有偶数个角度，且可以作为起始中间点的角度上一个有两个星体，另一个也有两个星体'
      Map<double,List<UIStarModel>> starsMapper = {};
      [
        UIStarModel(
      ...
      expect(result.orderedStars.isNotEmpty, true);
      expect(result.orderedStars.length, equals(5));
      expect(result.orderedStars.map((s)=>s.angle).toSet(), equals({5.0,9.0,13.0,17.0,21.0}));
    });
  ...
  group('doResolveFixRightEdgeAngle Tests',(){
    ...
    test("1",(){
     ...
     UIConstellationModel constellationModel = UIConstellationModel(orderedStars: stars);
     constellationModel.toLeftAdjustAngle(1);
     expect(constellationModel.orderedStars.map((s)=>s.angle).toSet(), equals({4,8,12,16}));
  ...
  group("doResolveConstellationWithStars Test",(){
    test("do test",(){
      ...
      double rangeAngleEachSide = 4;
      Tuple2<UIConstellationModel,List<UIStarModel>> result = StarsResolver.doResolveConstellationWithStars(
      ...
      expect(result.item1.orderedStars.map((s)=>s.angle).toSet(),equals({6,10,14,18}));
      expect(result.item2.map((s)=>s.star),equals({EnumStars.Golden,EnumStars.Wood}));
...
int calculateAdjustment(int ownStart, int ownEnd, int otherStart, int otherEnd) {
...
List<int> sortCircularAngles_int(List<int> angles){
  ...
  print(lists);
    ...
    // 锚点以左应该有的数字
    Map<int,int> _anchorLeftMapper = {};
    // 锚点以右应该有的数字
    Map<int,int> _anchorRightMapper = {};
    ...
    List<int> sortedLeftKeys = _anchorLeftMapper.keys.toList()..sort();

    List<int> sortedRightKeys = _anchorRightMapper.keys.toList()..sort();
    List<int> rightNumber = sortedLeftKeys.map((e)=>_anchorLeftMapper[e]!).toList();
    // print("~~~~~~");
    // print(sortedRightKeys);
    List<int> leftNumbers = sortedRightKeys.reversed.map((e)=>_anchorRightMapper[e]!).toList();
    // print(leftNumbers);
    // print(anchor);
    // print(rightNumber);
    //
