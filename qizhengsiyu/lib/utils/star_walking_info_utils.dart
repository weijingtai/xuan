...
class StarWalkingInfoUtils{
  static FiveStarWalkingInfo calculateStarWalkingInfo(
      ...
      Map<EnumStars,Tuple6<double,double,double,double,double?,double?>> mapper){
    ...
    var planetInfo = Sweph.swe_calc(julianDay, planetBody, SwephFlag.SEFLG_SWIEPH|SwephFlag.SEFLG_SPEED);

    // tuple6.item1 最快速度，item2 逆行最快，item3 逆行，item4 留行，item5疾行，item6 迟行


    int _speedStringFixed = 3;
    double observerAngle = double.parse(planetInfo.longitude.toStringAsFixed(2));
    double observerSpeed = double.parse(planetInfo.speedInLongitude.toStringAsFixed(_speedStringFixed));
    Tuple6<double,double,double,double,double?,double?> tuple6 = mapper[fiveStar]!;
    double maxSpeed = tuple6.item1;
    double retrogradeMaxSpeed = tuple6.item2;
    double timeStep = 2/24;
    FiveStarWalkingType currentFiveStarWalkingType = getWalkingType(observerSpeed, tuple6);

    List<FiveStarWalkingType> walkingSeqForward = fiveStar.fullForwardList;

    List<FiveStarWalkingType> reversedSeq = FiveStarWalkingType.reverseToPreviousListAndChangeFirst(currentFiveStarWalkingType, walkingSeqForward.reversed.toList());
    ...
    for (var i = 0; i < reversedSeq.length; i++) {
      StarWalkingInfo inf = prevWalkingType(fiveStar,planetBody,observerSpeed, reversedJulianDay, reversedStarWalkingType, tuple6, speedStringFixed: _speedStringFixed,timeStep: timeStep);
    ...
    List<FiveStarWalkingType> forwardSeq = FiveStarWalkingType.changeFirst(currentFiveStarWalkingType, walkingSeqForward);
    ...
    for (var i = 0; i < forwardSeq.length; i++) {
      StarWalkingInfo inf =nextWalkingType(fiveStar,planetBody,observerSpeed, forwardJulianDay, forwardStarWalkingType, tuple6, speedStringFixed: _speedStringFixed,timeStep: timeStep);
  ...
  static StarWalkingInfo prevWalkingType(
    ...
    while (prevForwardType == currentFiveStarWalkingType){
      var planetInfo = Sweph.swe_calc(prevForwardJD, planetBody, SwephFlag.SEFLG_SWIEPH|SwephFlag.SEFLG_SPEED);
      double speed = double.parse(planetInfo.speedInLongitude.toStringAsFixed(speedStringFixed));
      ...
      //   break;
      // }
      FiveStarWalkingType _type = getWalkingType(prevForwardSpeed, tuple6);
      DateTime _time = juldayToDatetime(prevForwardJD);
      if (prevForwardType == _type){
        prevForwardJD -= timeStep;
        continue;
      }
      prevForwardType = _type;
      prevForwardJD -= timeStep;
  ...
  static StarWalkingInfo nextWalkingType(
    ...
    while (prevForwardType == currentFiveStarWalkingType){
      var planetInfo = Sweph.swe_calc(prevForwardJD, planetBody, SwephFlag.SEFLG_SWIEPH|SwephFlag.SEFLG_SPEED);
      double _speed = double.parse(planetInfo.speedInLongitude.toStringAsFixed(speedStringFixed));
      if (_speed == prevForwardSpeed){
        prevForwardJD += timeStep;
        continue;
      }
      prevForwardSpeed = _speed;
      // print("object $prevForwardSpeed ${_speed == prevForwardSpeed}");
      // if (_speed>=maxSpeed||_speed<=retrogradeMaxSpeed){
      //   print("${juldayToDatetime(prevForwardJD)} ${planetInfo.longitude.toStringAsFixed(2)}° ${_speed}°/天 max");
      //   break;
  ...
  static double datetimeToJulday(DateTime utcTime){
    return Sweph.swe_julday(utcTime.year,utcTime.month,utcTime.day,utcTime.hour+utcTime.minute/60,CalendarType.SE_GREG_CAL);
  }
  static DateTime juldayToDatetime(double julianDay){
    return Sweph.swe_revjul(julianDay, CalendarType.SE_GREG_CAL);
  }
  static FiveStarWalkingType getWalkingType(double observerSpeed, Tuple6<double,double,double,double,double?,double?> tuple6){
    double maxSpeed = tuple6.item1;
    double retrogradeMaxSpeed = tuple6.item2;
