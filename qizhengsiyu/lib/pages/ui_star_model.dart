...
class UIStarModel extends Equatable with Compare<UIStarModel>, Edge {
  ...
  bool? previousAdjustedDirection = false; // null 为之前未进行调整， false表示前一次调整为向左“-”，true表示前一次调整为向右“+”
  ...
  UIStarModel generateEasyCalculate(){
    return UIStarModel(star: star,priority: priority, originalAngle: originalAngle+360, rangeAngleEachSide: rangeAngleEachSide);
  }
  UIStarModel clone() {
    // 先复制基本类型属性
    ...
    if (adjustedEdges != null) {
      cloned.adjustedEdges =
          Tuple3(adjustedEdges!.item1, adjustedEdges!.item2, adjustedEdges!.item3);
    }
  ...
  static Tuple3<double,double,double> getAngleRange(double angle,double minAngleDiff){
    // 需要处理0°与360°
  ...
  static double getMinDiffAngleOfTwoStar(UIStarModel star1,UIStarModel star2){
    return star1.rangeAngleEachSide >= star2.rangeAngleEachSide ? star1.rangeAngleEachSide : star2.rangeAngleEachSide;
  }
  ...
  @override Tuple3<bool,double?,double?>  inRangeAngle(Edge other){
  ...
  void adjustAngle(double addAngle) {
    ...
    adjustedEdges = Tuple3(correctCircleAngle(adjustedAngle!-rangeAngleEachSide),correctCircleAngle(adjustedAngle!+rangeAngleEachSide),adjustedAngle!);
  ...
  UIConstellationModel? addStar(UIStarModel other) {
    ...
    double otherStarEdgeRange = shortestAngleDifference(other.edges.item1, other.edges.item2);
    double thisStarEdgeRange = shortestAngleDifference(edges.item1, edges.item2);
...
class UIConstellationModel with Edge{
  ...
  UIConstellationModel({
    required this.orderedStars
  }){
  ...
  static Tuple3<double,double,double> caculateScalebleEdges(List<UIStarModel> stars){
    return Tuple3(
      stars.first.edges.item1,
      stars.last.edges.item2,
        calculateMidpointAngle(stars.first.edges.item1,stars.last.edges.item2));
  }
  static List<UIStarModel> sortStar(List<UIStarModel> stars){
    Map<double,List<UIStarModel>> _mapper = {};
    for (var star in stars) {
      if (_mapper.containsKey(star.angle)) {
        _mapper[star.angle]!.add(star);
      } else {
        _mapper[star.angle] = [star];
      }
    }
    List<double> sortedAngleResult = sortCircularAngles(stars.map((e)=>e.angle).toList());
