...
enum DiZhiTextLayoutStyle { vertical, triangle }

/// 神煞文字的朝向枚举
class Gong12DiZhiRing extends StatelessWidget {
  // 这个用于在UI层面显示 地支12宫，如“子坎土”，“丑艮土”
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: CustomMultiChildLayout(
        delegate: _ShenShaGongLayoutDelegate(
          itemCount: shenShaMapper.length,
          radius: 0, // 所有子项都从中心开始布局
          itemSize: itemSize,
        ),
        children: [
          for (int i = 0; i < shenShaMapper.length; i++)
            LayoutId(
              id: i,
              child: SizedBox(
                  width: itemSize,
                  ...
                  child: Transform.rotate(
                    ...
                    child: CustomPaint(
                      ...
                      painter: SectorPainter(
                        ...
                        color: Colors.teal[500]!.withAlpha(10 * (i + 1)),
                      ...
                      child: GongShenShaRing(
                        ...
                        innerRadius: innerRadius,
                        gongAngleOffset: 0,
                        textGongAngleOffset: i * 30,
                        shenShaList:
                            shenShaMapper[shenShaMapper.keys.toList()[i]]!,
                      ),
                    ),
                  )),
            ),
...
class GongShenShaRing extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: CustomMultiChildLayout(
        ...
        children: [
          for (int j = 0; j < shenShaList.length; j++)
            eachShenShaItem(
              j: j,
  ...
  Widget eachShenShaItem({
    ...
    // isOdd ? shenShaList.length ~/ 2 + 1 : shenShaList.length ~/ 2;
    double textSize = 16;
    double halfTextSize = textSize / 2;
    ...
    return LayoutId(
      ...
      child: _ShenShaItem(
        name: shenShaList[j],
        index: index,
        angleOffset: 0 * math.pi / 180,
        totalCount: shenShaList.length,
        outerRadius: _outerRadius,
        innerRadius: _innerRadius - ((shenShaList.length - j) * halfTextSize),
        itemSize: itemSize,
        ...
        // gongOffset: (gongAngleOffset - 30) * math.pi / 180,
        gongOffset: 0 * math.pi / 180,
      ),
    );
  }
  ...
  double calculateAngleOffset({
    ...
    double charWidth = middleRadius - innerRadius;
...
class _ShenShaItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double fontSize = (outerRadius - innerRadius) * 0.3;
    if (fontSize < 12) {
      fontSize = 12;
    }
    // TODO: DEV
    ...
    final double radialPosition = innerRadius;
    ...
    return SizedBox(
      ...
      child: Transform.rotate(
        ...
        child: CustomPaint(
          ...
          child: _buildShenShaContent(textOffset, textStyle, baseItemRotation),
        ),
      ),
    );
  }
  ...
  Widget _buildShenShaContent(
      Offset offset, TextStyle textStyle, double baseItemRotation) {
    return Transform.translate(
      offset: offset, // 定位文字容器的中心
      child: Transform.rotate(
        // 文字容器旋转，使其垂直于径向方向
        // angle:
        // math.pi / 2 + math.atan2(offset.dy, offset.dx) + baseItemRotation,
        // angle: -baseItemRotation,
        // angle: (index * angleOffset + 105) * math.pi / 180,
        // angle: (index * angleOffset + 105) * math.pi / 180,
        angle: 0,
  ...
  double _getTextRotationAngle() {
    switch (textDirection) {
      ...
      case RingTextDirection.gravity:
        // 文字垂直于半径，考虑宫位偏移
        return -(index * angleOffset + 105) * (math.pi / 180) - gongOffset;
      // return -(index * angleOffset + 90) * (math.pi / 180);
    }
  }
}
