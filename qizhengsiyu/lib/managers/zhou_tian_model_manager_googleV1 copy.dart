...
class CelestialObject {
  String name;
  double absStart; // 连续的绝对起始角度 (可能 > totalDegree)
  double absEnd; // 连续的绝对结束角度 (absStart + width)
  double width;
  CelestialObject(this.name, this.absStart, this.absEnd, this.width);

  @override
  String toString() =>
      '$name: [${absStart.toStringAsFixed(2)}°, ${absEnd.toStringAsFixed(2)}°) width ${width.toStringAsFixed(2)}°';
}
...
class ConstellationSegment {
  ...
  String toString() {
    String base =
        '  - In $palaceName: [${startInPalaceDeg.toStringAsFixed(2)}°-${endInPalaceDeg.toStringAsFixed(2)}°)] '
...
class ConstellationMappingResult {
  ...
  @override
  String toString() {
    return 'Constellation: $constellationName (Abs: [${absStartDeg.toStringAsFixed(2)}°-${absEndDeg.toStringAsFixed(2)}°), Width: ${totalWidthDeg.toStringAsFixed(2)}°)\n'
...
// 规范化角度到 [0, TOTAL_CELESTIAL_DEGREES)
  ...
  Map<EnumTwelveGong, CelestialObject> _calculatePalaceAngles() {
  ...
  Map<Enum28Constellations, CelestialObject> _calculateConstellationAngles() {
  ...
  List<ConstellationMappingResult> mapConstellationsToPalaces() {
    final Map<EnumTwelveGong, CelestialObject> palacesData =
        _calculatePalaceAngles();
    final Map<Enum28Constellations, CelestialObject> constellationsData =
        _calculateConstellationAngles();

    List<ConstellationMappingResult> results = [];

    List<CelestialObject> sortedPalaces = palacesData.values.toList();
    // 排序时使用规范化后的起始点，以确保正确的查找顺序
    sortedPalaces.sort((a, b) =>
        normalizeAngle(a.absStart, zhouTianModel.totalDegree)
            .compareTo(normalizeAngle(b.absStart, zhouTianModel.totalDegree)));

    for (Enum28Constellations cName in zhouTianModel.starInnOrder) {
      CelestialObject constellation = constellationsData[cName]!;

      // 将星宿的原始double值转为整数进行计算
      int constellationStartContInt =
          (constellation.absStart * DEGREE_MULTIPLIER).round();
      ...
      while (constellationProcessedDegreesInt <
        ...
        for (CelestialObject p in sortedPalaces) {
          int pStartContInt = (p.absStart * DEGREE_MULTIPLIER).round();
          int pEndContInt =
              (p.absEnd * DEGREE_MULTIPLIER).round(); // p.absStart + p.width
        ...
        int palaceStartContInt =
            (currentPalace.absStart * DEGREE_MULTIPLIER).round();
        ...
        segments.add(ConstellationSegment(
          ...
          startInConstellationDeg:
              constellationProcessedDegreesInt / DEGREE_MULTIPLIER, // 这是关键
      ...
      results.add(ConstellationMappingResult(
        ...
        absStartDeg:
            normalizeAngle(constellation.absStart, zhouTianModel.totalDegree),
        absEndDeg:
            normalizeAngle(constellation.absEnd, zhouTianModel.totalDegree),
