  ...
  group("十干克应，2",()
  {
    test("ten gan ke ying", () {
      ...
      try{
        ...
        final file2 = File('../assets/qi_men_dun_jia/ten_gan_ke_ying_ji_xiong.json');
        ...
        for(var key in jsonMapper2.keys) {
          ...
          for (var k in jsonMapper2[key]!.keys) {
            ...
            Set<String> geJuNameList = geJuNames.map((e) => e.toString())
                .toSet();
            if (keYing?.juName != null) {
              geJuNameList.add(keYing?.juName ?? "");
            }
            List<String> explainList = shorExplains.map((e) => e.toString())
                .toList();
