...
void main() {
  ...
  group("置润法 数格子法确定阴遁局数", (){
    ...
    test("2025-06-24 夏至：上元第1天 阴遁九局",(){
      ...
      Tuple4<int,EnumThreeYuan,TwentyFourJieQi,int> result = zhirun.doCa(targetDateTime);

      print("${result.item1} ${result.item2.name} ${result.item3.name} ${result.item4}");
  ...
  group("depected", (){
    ...
    test("置润 正授过后 >= 360天之内 阳遁 2023-12-21 是年大雪最后一天（冬至前一天），这天使用“阳遁”冬至上元1局第5天（此元最后一天）",(){
    ...
    test("置润 正授过后 180~360天之内 阴遁 2023-06-20 是年芒种最后一天（夏至前一天），这天夏至阴遁使用上元9局1天（即，阴遁第一局第一天）",(){
    ...
    test('置润法 2', () {
      ...
      DateTime otherJieQiDateTime = dateFormatter.parse(lunar.getJieQiTable()[targetJieQiSt]!.toYmdHms());
      // 23点之后调整为第二天
      ...
      int toTargetDiffDays = otherJieQiDateTime.difference(zhengShouDongZhi).inDays;
      print("正授冬至 到 目标节气 $targetJieQiSt 共 $toTargetDiffDays 天");
      DateTime targetJieQiDateTime = zhengShouDongZhi.add(Duration(days: toTargetDiffDays));
      print("${Lunar.fromDate(targetJieQiDateTime).getJieQi()} $targetJieQiDateTime");
      ...
      if (toTargetDiffDays > 180){
        ...
        print("夏至阴遁将会提前${startBeforeDays}开始（超神）");
        xiaZhiYinDunStartAt = targetJieQiDateTime.subtract(Duration(days:startBeforeDays));
      ...
      Iterable<Tuple3<int,EnumThreeYuan,TwentyFourJieQi>> newList = ShiJiaQiMenJuCalculator.yinDunList;
      print("目标日期，已经是阴遁开始后的第${startBeforeDays+1}天");

      print("${newList.first.item3.name} ${newList.first.item2.name}第${startBeforeDays+1}天 第${newList.first.item1}局 ");
    ...
    test('置润法 1', () {
      ...
      Lunar lunar = Lunar.fromDate(DateTime(zhengShouDongZhi.year+1,9,12,12,0,0));
      ...
      Iterable<Tuple3<int,EnumThreeYuan,TwentyFourJieQi>> newList = ShiJiaQiMenJuCalculator.yangDunList.skip(fullXun * 3);
      print("${newList.first.item3.name} ${newList.first.item2.name} ${newList.first.item1}");
