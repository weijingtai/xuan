...
class QiMenDunJiaGong{
  ...
  // 构造方法
  QiMenDunJiaGong({
    required this.id, required this.gua,
    this.diPan,
    this.diPan_ji,
    this.tianPan,
    this.tianPan_ji,
...
class QiMenDunJiaPlate {
  ...
  static final DEFAULT_SAN_QI_LIU_YI_SEQ = ["戊","己","庚","辛","壬","癸","丁","丙","乙"];
  // DEFAULT_NINE_STAR_SEQ
  static final DEFAULT_NINE_STAR_SEQ = ["天蓬星", "天任星", "天冲星", "天辅星", "天英星", "天芮星", "天柱星", "天心星"];
  // DEFAULT_EIGHT_GOD_SEQ
  static final DEFAULT_EIGHT_GOD_SEQ = ["值符","螣蛇","太阴","六合","白虎","玄武","九地","九天"];
  // DEFAULT_TIAN_GAN_SEQ
  static final DEFAULT_TIAN_GAN_SEQ = ["甲","乙","丙","丁","戊","己","庚","辛","壬","癸"];
  // DEFAULT_EIGHT_DOOR_SEQ
  static final DEFAULT_EIGHT_DOOR_SEQ = ["休门","生门","伤门","杜门","景门","死门","惊门","开门"];
  // CLOCKWISE_ORDER
  ...
  static final List<List<QiMenDunJiaGong>> QI_MEN_GONG_TEMPLATE = [
    [QiMenDunJiaGong(id: 4, gua: "巽"),QiMenDunJiaGong(id: 9, gua: "离"),QiMenDunJiaGong(id: 2, gua: "坤")],
    [QiMenDunJiaGong(id: 3, gua: "震"),QiMenDunJiaGong(id: 5, gua: "中"),QiMenDunJiaGong(id: 7, gua: "兑")],
    [QiMenDunJiaGong(id: 8, gua: "艮"),QiMenDunJiaGong(id: 1, gua: "坎"),QiMenDunJiaGong(id: 6, gua: "乾")],
  ];
  // 元一宫，值符，值使门
  //  index 是后天八卦数
  ...
  final Map<String,Tuple4<int,int,int,bool>> JIE_QI_JU_MAPPER = {
    "冬至":Tuple4(1,7,4,true),
    "惊蛰":Tuple4(1,7,4,true),
    "小寒":Tuple4(2,8,5,true),
    "大寒":Tuple4(3,9,6,true),
    "春分":Tuple4(3,9,6,true),
    "雨水":Tuple4(9,6,3,true),
    "清明":Tuple4(4,1,7,true),
    "立夏":Tuple4(4,1,7,true),
    "立春":Tuple4(8,5,2,true),
    "谷雨":Tuple4(5,2,8,true),
    "小满":Tuple4(5,2,8,true),
    "芒种":Tuple4(6,3,9,true),

    "夏至":Tuple4(9,3,6,false),
    "白露":Tuple4(9,3,6,false),
    "小暑":Tuple4(8,2,5,false),
    "大暑":Tuple4(7,1,4,false),
    "秋分":Tuple4(7,1,4,false),
    "立秋":Tuple4(2,5,8,false),
    "寒露":Tuple4(6,9,3,false),
    "立冬":Tuple4(6,9,3,false),
    "处暑":Tuple4(1,4,7,false),
    "霜降":Tuple4(5,8,2,false),
    "小雪":Tuple4(5,8,2,false),
    "大雪":Tuple4(4,7,1,false),
  };
  ...
  // key 是后天八卦数
  Map<int,QiMenDunJiaGong> _houTianNumber_gongMapper = {};
  // key 是地盘三奇六仪
  Map<String,QiMenDunJiaGong> _diPan_gongMapper = {};
  ...
  List<List<QiMenDunJiaGong>> arrangeTurnPlate(DateTime dateTime){
    ...
    List<List<QiMenDunJiaGong>> ju = QI_MEN_GONG_TEMPLATE.map((e) => e.map((e) => QiMenDunJiaGong(id: e.id, gua: e.gua,)).toList()).toList();
    ...
    // 3.2. 根据旬首，旬首所处的六仪
    String xunShouYi = SIX_JIA.firstWhere((e) => e.startsWith(timeXunShou)).substring(2);
    // 3.3. 根据六仪，获取六仪的顺序
    ...
    List<Tuple2<int,String>> yiJuList = IterableZip([juList,DEFAULT_SAN_QI_LIU_YI_SEQ]).map((p) => Tuple2<int,String>(p[0] as int, p[1] as String)).toList();
    // 根据 xunShouYi 找到对应的
  ...
  void feiPan(int startFromId, String xing,List<List<QiMenDunJiaGong>> pan){
    ...
    List<int> gongSeq = ArrangePlateUtils.turingListSeq(CLOCKWISE_ORDER,startFromId);

    List<String> nineStarSeq = ArrangePlateUtils.turingStringListSeq(DEFAULT_NINE_STAR_SEQ,xing);
    // zip nineStarSeq 与 seq
    List<Tuple2<int,String>> nineStarSeq1 = IterableZip([gongSeq,nineStarSeq]).map((p) => Tuple2<int,String>(p[0] as int, p[1] as String)).toList();
  ...
  void tianPanJiGong(){
    // 找到存在“地盘寄宫”的地盘天干
    QiMenDunJiaGong diPanJiGong = _houTianNumber_gongMapper.values.firstWhere((e) => e.diPan_ji != null);
    // 找到天盘中对应这个 diPanJiGong.diPan
    QiMenDunJiaGong tianPanJiGong = _houTianNumber_gongMapper.values.firstWhere((e) => e.tianPan == diPanJiGong.diPan);
  ...
  void dingTianPan(int gongIndex){
    ...
    List<int> tianPanTimeList = ArrangePlateUtils.turingListSeq(CLOCKWISE_ORDER, diPanId);
    debugPrint("$diPanId $tianPanTimeList");
    // 6.4. 从旬首所在宫顺时针获取所有地支g
    List<int> _tmpTianPanTimeList = ArrangePlateUtils.turingListSeq(CLOCKWISE_ORDER,gongIndex);
    List<String> tianPanSeq = _tmpTianPanTimeList.map((e) => _houTianNumber_gongMapper[e]!.diPan!).toList();
    // zip tianPanSeq 与 tianPanTimeList
    // List<Tuple2<int,String>> tianPanSeq1 = zipTwoList(tianPanTimeList,tianPanSeq);
    List<Tuple2<int,String>> tianPanSeq1 = IterableZip([tianPanTimeList,tianPanSeq]).map((p) => Tuple2<int,String>(p[0] as int, p[1] as String)).toList();
    // 根据 tianPanSeq1 放入
    tianPanSeq1.forEach((t) {
      _houTianNumber_gongMapper[t.item1]!.tianPan = t.item2;
    });
  }

  // 排八神
  void dingEightShen(int startFromId, List<List<QiMenDunJiaGong>> pan,bool isYangDun){
    List<int> seq = isYangDun?CLOCKWISE_ORDER.toList(growable: true):ANTICLOCKWISE_ORDER.toList(growable: true);
    ...
    List<Tuple2<int,String>> nineStarSeq1 = IterableZip([seq,DEFAULT_EIGHT_GOD_SEQ]).map((p) => Tuple2<int,String>(p[0] as int, p[1] as String)).toList();
    nineStarSeq1.forEach((t) {
      // 根据给定的id，找到对应的宫位
  ...
  String dingDoor(String xunShouTianGan, List<List<QiMenDunJiaGong>> pan){
    ...
    // 5.2. 根据地支，找到对应的八门
    List<String> seq = ArrangePlateUtils.turingStringListSeq(DEFAULT_SAN_QI_LIU_YI_SEQ,xunShouTianGan);
    debugPrint("!!!!!!!!!!!!!!!!!!!!!!!!!!");
  ...
  void arrangeDoor(String zhiShiDoor,String zhiShiDoorGan, List<List<QiMenDunJiaGong>> pan,bool isYangDun){
    ...
    List<String> doorList = ArrangePlateUtils.turingStringListSeq(DEFAULT_EIGHT_DOOR_SEQ,zhiShiDoor);
    debugPrint(doorList.toString());
    List<int> gongIndexSeq = ArrangePlateUtils.turingListSeq(CLOCKWISE_ORDER, firstDoorInGongIndex);
    ...
    List<Tuple2<int,String>> gongIndexSeq1 = IterableZip([gongIndexSeq,doorList]).map((p) => Tuple2<int,String>(p[0] as int, p[1] as String)).toList();
  ...
  void tianQinStarJiGong_1(List<List<QiMenDunJiaGong>> ju){
    // 找到天芮星所在宫位并将“天禽星”设置上
    QiMenDunJiaGong tianQinGong = _houTianNumber_gongMapper.values.firstWhere((e) => e.nineStar == "天芮星");
  ...
  void dingYinPan_1(String zhiShiDoor,List<List<QiMenDunJiaGong>> ju,bool isYangDun){
    ...
    QiMenDunJiaGong zhiShiGong = zhiShiGongList.firstWhere((e) => e.eightDoor == zhiShiDoor);
    ...
    List<int> juList = ArrangePlateUtils.yangShunYinNiSeq(fromGongNumber, isYangDun);

    List<String> ganList = ArrangePlateUtils.turingStringListSeq(DEFAULT_SAN_QI_LIU_YI_SEQ, timeGan);
    ...
    List<Tuple2<int,String>> ganJuList = IterableZip([juList,ganList]).map((p) => Tuple2<int,String>(p[0] as int, p[1] as String)).toList();
