...
class ShiJiaQiMenViewModel extends ChangeNotifier{
  ...
  UIEachGongModel? _generateEachGong(HouTianGua gua,UITenGanKeYingGeJu tenGanKeYingGeJu,List<QiYiRuGong>? qiYiRuGongList){
    if (shiJiaQiMen != null){
      return UIEachGongModel(
          ...
          eachGongGeJu: EnumMostPopularGeJu.checkGeJuAtEachGong(_shiJiaQiMen!.timeJiaZi, _shiJiaQiMen!.sixJiaXunHeader,_shiJiaQiMen!.zhiShiDoor,_shiJiaQiMen!.gongMapper[gua]!),
  ...
  Future<void> selectGong(HouTianGua? gongGua) async {
    if (_selectedGong == null && _gongUIMapper[gongGua] != null){
      ...
      var fixedList = [
        loadAllTenGanKeYingForCurrentGong(_uiPanMetaModel!.xunHeaderTianGan,gong.tianPan,gong.diPan,gong.tianPanJiGan,gong.diPanJiGan),
        ...
        loadEightDoorKeYing(gong.door,EightDoorEnum.listOrderedByGongNumber.firstWhere((t)=>t.originalGong == gong.gongGua))
      ];
      ...
      EachGongGeJu eachGongGeJu = EnumMostPopularGeJu.checkGeJuAtEachGong(_shiJiaQiMen!.timeJiaZi, _shiJiaQiMen!.sixJiaXunHeader,_shiJiaQiMen!.zhiShiDoor,gong);
      Future.wait(fixedList).then((values){
        _selectedGongExplain = UIGongExplains(
          ...
          doorJiGanKeYingString:withTianGanJiGan?values[6] as String?:null,
          jiGanThreeRuGong: withTianGanJiGan?values[7] as QiYiRuGong?:null,
  ...
  void createShiJiaQiMen(PlateType plateType,DateTime dateTime,ShiJiaJu shiJiaJu,PanArrangeSettings settings){
    // _dateTime = dateTime;
    List<String> _eightCharList = shiJiaJu.fourZhuEightChar.split(" ").toList();
    ...
    // 三奇入宫的三奇 与 宫Mapper
    Map<TianGan,HouTianGua> _sanQiRuGongMapper = {};
    shiJiaQiMen.gongMapper.values.forEach((g){
      if (g.tianPan.isThreeQi) {
        _sanQiRuGongMapper[g.tianPan] = g.gongGua;
      }
      if (g.tianPanJiGan != null && g.tianPanJiGan!.isThreeQi){
        _sanQiRuGongMapper[g.tianPanJiGan!] = g.gongGua;
      }
    });

    Future.wait([
      loadTenGanKeYingGeJu(plateType, shiJiaQiMen.xunHeaderTianGan,shiJiaQiMen.gongMapper),
      listThreeQiRuGong(_sanQiRuGongMapper)
    ])
        .then((resList){
      print("Logic: ten gan ke ying loadded ${resList.first.length}");
      HouTianGua.values.forEach((gua){
        if (gua == HouTianGua.Center && plateType == PlateType.ZHUAN_PAN){
        }else{
          _gongUIMapper[gua] = _generateEachGong(gua,
              resList.first[gua] as UITenGanKeYingGeJu,
            resList[1][gua] == null ?null: (resList[1][gua] as List<QiYiRuGong>),
          );
        }
      });
      notifyListeners();
  ...
  Future<TenGanKeYing?> loadTenGanKeyYing(TianGan tianPanGan, TianGan diPanGan) async {
  ...
  Future<Map<HouTianGua,List<QiYiRuGong>>> listThreeQiRuGong(Map<TianGan,HouTianGua> mapper) async {
    Map<HouTianGua, Map<TianGan, QiYiRuGong>> qiYiRuGongMapper = await ReadDataUtils.readQiYiRuGong();
    Map<HouTianGua,List<QiYiRuGong>> res = {};
    for (var mapperEntry in mapper.entries){
      if (!res.containsKey(mapperEntry.value)){
        res[mapperEntry.value] = [qiYiRuGongMapper[mapperEntry.value]![mapperEntry.key]!];
      }else{
        res[mapperEntry.value]!.add(qiYiRuGongMapper[mapperEntry.value]![mapperEntry.key]!);
      }
    }
  ...
  Future<Map<HouTianGua,UITenGanKeYingGeJu>> loadTenGanKeYingGeJu(PlateType plateType, TianGan xunShouGan,Map<HouTianGua,EachGong> gong) async {
