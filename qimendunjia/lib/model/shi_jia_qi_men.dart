...
class ShiJiaQiMen{
  ...
  static final List<TianGan> siiYiThreeYiList = [TianGan.WU,TianGan.JI,TianGan.GENG,TianGan.XIN,TianGan.REN,TianGan.GUI,TianGan.DING,TianGan.BING,TianGan.YI];
  ...
  Map<HouTianGua,EachGong> calculateFeiPan(){
    ...
    Map<TianGan,int> currentJunGanGongMapper = arrangeJu(juNumber,currentPanYinYang);

    ganAtCenterGong = currentJunGanGongMapper.entries.firstWhere((e)=>e.value == 5).key;
    print ("当前中宫天干 ${ganAtCenterGong.name}");
    // print(currentJunGanGongMapper.entries.map((e)=>"${e.key.name}${e.value}").toList().join(" "));
    // 3. 根据天干统帅找到对应的后天八卦宫数
    // 确定天干统帅所在宫位
    print("当前为${yinYangDun.isYang?"阳":"阴"} ${juNumber}局");
    ...
    var zhuanPanSeqNew = ChangeSequenceUtils.changeNumberSeq(zhiFuAtGongNumber, yinYangDun.isYang?feiPanSeq:feiPanSeq.reversed.toList());
    ...
    List<NineStarsEnum> forCurrentPanSeq = ChangeSequenceUtils.changeNineStarsSeq(zhiFuStar, NineStarsEnum.listFeiPanOrderedByGongNumber);
    // print(star.name);
    print(forCurrentPanSeq.map((e)=>e.name).toList());
    Map<int,NineStarsEnum> _nineStarsGongNumberMapper = Map.fromIterables(zhuanPanSeqNew, forCurrentPanSeq);
    print(_nineStarsGongNumberMapper.entries.map((e)=>"${e.key}${e.value.name}"));
    Map<int,EightGodsEnum> _tianPanGodsGongNumberMapper = Map.fromIterables(zhuanPanSeqNew, currentPanYinYang.isYang?EightGodsEnum.feiPanYangDunList:EightGodsEnum.feiPanYinDunList);
    print(_tianPanGodsGongNumberMapper.entries.map((e)=>"${e.key}${e.value.name}"));
    ...
    if (currentPanYinYang.isYang){
      var res = ChangeSequenceUtils.changeNumberSeq(xunShouAtGongNumber,List.generate(9, (i)=>i+1));
      res = [...res, res.first];
    ...
    List<int> _zhuanPanDaoSeq = ChangeSequenceUtils.changeNumberSeq(zhiShiDoorAtGongNumber, zhuanPanSeqNew);
    List<EightDoorEnum> currentEightDoorSeq = ChangeSequenceUtils.changeDoorSeq(zhiShiDoor, EightDoorEnum.listOrderedByGongNumber);
    Map<int,EightDoorEnum> _zhuanPanEigthDoorMapper = Map.fromIterables(_zhuanPanDaoSeq,currentEightDoorSeq);
    print(_zhuanPanEigthDoorMapper.entries.map((e)=>"${e.key}${e.value.name}"));

    // 确定地盘天干
    // currentJunMapper key 变 value， value 变 key
    Map<int,TianGan> _diPanGanWithGongMapper = Map.fromEntries(currentJunGanGongMapper.entries.map((e)=>MapEntry(e.value, e.key)));
    print("地盘：${_diPanGanWithGongMapper.entries.map((e)=>"${e.key}${e.value.name}")}");
    ...
    List<int> diPanTianGanZhuanIndexSeq = ChangeSequenceUtils.changeNumberSeq(xunShouAtGongNumber, zhuanPanSeqNew);
    List<TianGan> tianPanTianGanSeq = [];
    for (var i = 0;i < diPanTianGanZhuanIndexSeq.length;i++){
      tianPanTianGanSeq.add(_diPanGanWithGongMapper[diPanTianGanZhuanIndexSeq[i]]!);
    }
    // print(tianPanTianGanSeq.map((e)=>e.name));
    Map<int,TianGan> _tianPanTianGanMapper = Map.fromIterables(zhuanPanSeqNew, tianPanTianGanSeq);
    print("天盘：${_tianPanTianGanMapper.entries.map((e)=>"${e.key}${e.value.name}")}");
    ...
    int diZiFuGodAtGongIndex = _diPanGanWithGongMapper.entries.firstWhere((entry)=>entry.value == xunHeaderTianGan).key;
    List<int> diGodAtGongSeq = ChangeSequenceUtils.changeNumberSeq(diZiFuGodAtGongIndex, feiPanSeq);
    if (yinYangDun.isYin){
      diGodAtGongSeq = [diGodAtGongSeq.first,...diGodAtGongSeq.skip(1).toList().reversed];
    }
    // 八神
    Map<int,EightGodsEnum> _diGodsGongNumberMapper = Map.fromIterables(diGodAtGongSeq, yinYangDun.isYang?EightGodsEnum.feiPanYangDunList:EightGodsEnum.feiPanYinDunList);
    print("地盘神：${_diGodsGongNumberMapper.entries.map((e)=>"${e.key}${e.value.name}")}");


    // 天盘隐干
    Map<int,TianGan> _tianPanAnGanMapper = orderTianPanAnGan(_nineStarsGongNumberMapper,_diPanGanWithGongMapper);
    print("天盘隐干：${_tianPanAnGanMapper.entries.map((e)=>"${e.key}${e.value.name}")}");
    // 人盘隐干
    Map<int,TianGan> _renPanAnGanMapper = orderRenPanAnGan(_zhuanPanEigthDoorMapper, _diPanGanWithGongMapper);
    print("人盘隐干：${_renPanAnGanMapper.entries.map((e)=>"${e.key}${e.value.name}")}");

    Map<int, TianGan> _yinGanMapper = orderYinGan(_diPanGanWithGongMapper);
    print("隐干：${_yinGanMapper.entries.map((e)=>"${e.key}${e.value.name}")}");
    ...
    for (int i = 1;i < 10;i++){
      ...
      gongRes[HouTianGua.getGua(i)] = generateEachGong(
          i: i,
          nineStarsGongNumberMapper: _nineStarsGongNumberMapper,
          zhuanPanEigthDoorMapper: _zhuanPanEigthDoorMapper,
          eightGodsGongNumberMapper: _tianPanGodsGongNumberMapper,
          tianPanTianGanMapper: _tianPanTianGanMapper,
          tianPanAnGanMapper: _tianPanAnGanMapper,
          renPanAnGanMapper: _renPanAnGanMapper,
          yinGanMapper:_yinGanMapper,
          houGuaNumberGanMapper: _diPanGanWithGongMapper,
          diPanEightGodsMapper: _diGodsGongNumberMapper);
    }
  ...
  Map<HouTianGua,EachGong> calculateZhuanPan(){
    ...
    // 根据制度落宫找到转盘的八神开始的位置
    var zhuanPanSeqNew = ChangeSequenceUtils.changeNumberSeq(zhiFuAtGongNumber, zhuanPanSeq);

    NineStarsEnum star = zhiFuStar;
    List<NineStarsEnum> forCurrentPanSeq = ChangeSequenceUtils.changeNineStarsSeq(star, NineStarsEnum.listOrderedByClockwiseWithoutYing);
    Map<int,NineStarsEnum> _nineStarsGongNumberMapper = Map.fromIterables(zhuanPanSeqNew, forCurrentPanSeq);
    Map<int,EightGodsEnum> _tianPanGodsGongNumberMapper = Map.fromIterables(zhuanPanSeqNew, currentPanYinYang.isYang?EightGodsEnum.yangDunList:EightGodsEnum.yinDunList);
    // 根据当前是时辰干支与六甲旬首间隔的顺序，再根据阳顺阴逆从天干统帅，顺后天八卦宫顺序确定值使门位置
    int totalStep = shiJiaZi.number - xunShou.number;
    // 旬首所在后天八卦宫位
    int xunShouAtGongNumber = currentJunGanGongMapper[xunHeaderTianGan]!;
    ...
    List<int> _zhuanPanDaoSeq = ChangeSequenceUtils.changeNumberSeq(zhiShiDoorAtGongNumber, zhuanPanSeqNew);
    List<EightDoorEnum> currentEightDoorSeq = ChangeSequenceUtils.changeDoorSeq(zhiShiDoor, EightDoorEnum.listOrderedByClockedwiseWithoutCenter);
    Map<int,EightDoorEnum> _zhuanPanEigthDoorMapper = Map.fromIterables(_zhuanPanDaoSeq,currentEightDoorSeq);
    ...
    // 地盘八神，地盘值符神加临在地盘旬首所落的宫位上
    Map<int,EightGodsEnum> _diGodsGongNumberMapper = orderDiPanEightGods(_diPanGanWithGongMapper);


    // 天盘隐干
    Map<int,TianGan> tianPanAnGanMapper = orderTianPanAnGan(_nineStarsGongNumberMapper,_diPanGanWithGongMapper);
    // 人盘隐干
    Map<int,TianGan> renPanAnGanMapper = orderRenPanAnGan(_zhuanPanEigthDoorMapper, _diPanGanWithGongMapper);
    Map<int, TianGan> yinGanMapper = orderYinGan(_diPanGanWithGongMapper);
    ...
    for (int i = 1;i < 10;i++){
      ...
      gongRes[HouTianGua.getGua(i)] = generateEachGong(
          ...
          houGuaNumberGanMapper: _diPanGanWithGongMapper, diPanEightGodsMapper: _diGodsGongNumberMapper);
    }
  ...
  Map<int,TianGan> orderRenPanAnGan(Map<int,EightDoorEnum> doorsMapper,Map<int,TianGan> _diPanGanMapper){
    Map<int,TianGan> result = {};
    for (var k in doorsMapper.keys){
      EightDoorEnum god = doorsMapper[k]!;
      result[k] = _diPanGanMapper[god.originalGong.houTianOrder]!;
    }
    return result;
  }
  ...
  Map<int,TianGan> orderTianPanAnGan(
      Map<int,NineStarsEnum> nineStarMapper,
  ...
  Map<int,TianGan> orderYinGan(Map<int,TianGan> _diPanGanMapper){
    // 阳遁 三奇六仪 顺序
    ...
    if (startAtGongNumber != 5 && _diPanGanMapper[zhiShiDoor] == tianGan){
      //暗干与值使门落宫的地盘奇仪相同
      // 将时干入中宫，按照阳顺阴逆的宫序飞布“戊己庚辛壬癸丁丙乙”，
      startAtGongNumber = 5;
    }
    if (yinYangDun.isYang){
      nineGongSeq = ChangeSequenceUtils.changeNumberSeq(startAtGongNumber,List.generate(9, (i)=>i+1));
      nineGongSeq = [...nineGongSeq, nineGongSeq.first];
    }else{
      // 阴遁
      nineGongSeq = ChangeSequenceUtils.changeNumberSeq(startAtGongNumber,List.generate(9, (i)=>i+1));
      nineGongSeq = [nineGongSeq.first, ...nineGongSeq.skip(1).toList().reversed, nineGongSeq.first];
    }
    // 戊己庚辛壬癸丁丙乙
    List<TianGan> newList = ChangeSequenceUtils.changeThreeQiXiYiSeq(tianGan, siiYiThreeYiList);
  ...
  /// 排地盘八神
  Map<int,EightGodsEnum> orderDiPanEightGods(Map<int,TianGan> _diPanGanWithGongNumber){
    int diZiFuGodAtGongIndex = _diPanGanWithGongNumber.entries.firstWhere((entry)=>entry.value == xunHeaderTianGan).key;
    if (diZiFuGodAtGongIndex == 5){
      diZiFuGodAtGongIndex = jiGong.getJiGong(yinYangDun, jieQi).index;
    }
    List<int> diGodAtGongSeq = ChangeSequenceUtils.changeNumberSeq(diZiFuGodAtGongIndex, zhuanPanSeq);
    // 八神
    return Map.fromIterables(diGodAtGongSeq, yinYangDun.isYang?EightGodsEnum.yangDunList:EightGodsEnum.yinDunList);
  ...
  EachGong generateEachGong({
    ...
    EachGong result =  EachGong(
      ...
      sixJiaXunHeader: houGuaNumberGanMapper[i]! == xunHeaderTianGan ? sixJiaXunHeader : null,
    );
  ...
  Map<HouTianGua,EachGongWangShuai> calculateEachGongWangShuai(Map<HouTianGua,EachGong> gongRes){
    // 一、不包括伏吟、反吟的情况
    ...
    return Map.fromEntries(gongRes.entries.map((e)=>MapEntry(e.key, generateEachGongWangShuai(e.key, e.value))));
  }
  EachGongWangShuai generateEachGongWangShuai(HouTianGua gongGua,EachGong each){
    ...
    return EachGongWangShuai(
      ...
      starMonthWangShuai:starMonthTokenType == MonthTokenTypeEnum.ZHU_QI_NA_GUA?each.star.checkWithMonthTokenNaGua(monthToken):each.star.checkWithMonthToken(monthToken),
      starMonthTokenType:starMonthTokenType,
      starGongWangShuai:starFourWeiGongType==GongTypeEnum.GONG_GUA?each.star.checkWithGongGua(gongGua):each.star.checkWithGongNeiDiZhi(gongGua, yinYangDun, timeJiaZi, starFourWeiGongType),
      starFourWeiGongType:starFourWeiGongType,

      doorFuFanYin: each.door.checkFuFanYinByGong(gongGua),
      doorMonthWangShuai:each.door.checkWithMonthToken(monthToken), // 固定使用月令主气五行
      doorGongWangShuai:each.door.checkWithGong(gongGua), // 有不同的方式进行
      doorFourWeiGongType:doorFourWeiGongType,
      doorGongRelationship:GongAndDoorRelationship.getRelationship(each.door, gongGua),
      isDoorRuMu: NineYiUtils.isDoorRuMu(each.door, gongGua),
      godGongWangShuai:godWithGongTypeEnum == GodWithGongTypeEnum.GONG_GUA_ONLY?each.god.checkWangShuaiWithGongGua(gongGua):each.god.checkWangShuaiWithDiPanGanNaGua(each.diPan),
      godWithGongType:godWithGongTypeEnum,

      diPanMonthZhangSheng:TwelveZhangSheng.getZhangShengByTianGanDiZhi(isDiPanXunShou?TianGan.JIA:each.diPan, monthToken.diZhi),
      tianDiPanGanRelationship:FiveXingRelationship.checkRelationship(isTianPanXunShou?TianGan.JIA.fiveXing:each.tianPan.fiveXing, isDiPanXunShou?TianGan.JIA.fiveXing:each.diPan.fiveXing)!, // 地盘干与天盘干
      tianPanMonthZhangSheng:TwelveZhangSheng.getZhangShengByTianGanDiZhi(isTianPanXunShou?TianGan.JIA:each.tianPan, monthToken.diZhi),
      tianPanJiGanMonthZhangSheng: each.tianPanJiGan == null?null:TwelveZhangSheng.getZhangShengByTianGanDiZhi(isTianPanXunShou?TianGan.JIA:each.tianPanJiGan!, monthToken.diZhi),
      diPanJiGanMonthZhangSheng: each.diPanJiGan == null?null:TwelveZhangSheng.getZhangShengByTianGanDiZhi(isTianPanXunShou?TianGan.JIA:each.diPanJiGan!, monthToken.diZhi),
      tianPanAnGanMonthZhangSheng:TwelveZhangSheng.getZhangShengByTianGanDiZhi(each.tianPanAnGan, monthToken.diZhi),
      renPanAnGanMonthZhangSheng:TwelveZhangSheng.getZhangShengByTianGanDiZhi(each.renPanAnGan, monthToken.diZhi),
      yinGanMonthZhangSheng:TwelveZhangSheng.getZhangShengByTianGanDiZhi(each.yinGan, monthToken.diZhi),

      tianPanGongZhangSheng:NineYiUtils.qiYiZhangSheng(timeJiaZi, each.tianPan, gongGua, yinYangDun, settings.ganGongType),

      tianPanAnGanGongZhangSheng:NineYiUtils.qiYiZhangSheng(timeJiaZi, each.tianPanAnGan, gongGua, yinYangDun, settings.ganGongType),
      diPanGongZhangSheng:NineYiUtils.qiYiZhangSheng(timeJiaZi, each.diPan, gongGua, yinYangDun, settings.ganGongType),
      renPanAnGanGongZhangSheng:NineYiUtils.qiYiZhangSheng(timeJiaZi, each.renPanAnGan, gongGua, yinYangDun, settings.ganGongType),
      yinGanGongZhangSheng:NineYiUtils.qiYiZhangSheng(timeJiaZi, each.yinGan, gongGua, yinYangDun, settings.ganGongType),
      tianPanJiGanGongZhangSheng: each.tianPanJiGan == null ?null:NineYiUtils.qiYiZhangSheng(timeJiaZi, each.tianPanJiGan!, gongGua, yinYangDun, settings.ganGongType),
      diPanJiGanGongZhangSheng: each.diPanJiGan == null ?null:NineYiUtils.qiYiZhangSheng(timeJiaZi, each.diPanJiGan!, gongGua, yinYangDun, settings.ganGongType),

        diPanJiGanGongIsMuOrKu:each.diPanJiGan==null?null:NineYiUtils.checkMuOrKu(monthToken, each.diPanJiGan!, gongGua),
        tianPanJiGanGongIsMuOrKu:each.tianPanJiGan==null?null:NineYiUtils.checkMuOrKu(monthToken, each.tianPanJiGan!, gongGua),
        diPanGongIsMuOrKu:NineYiUtils.checkMuOrKu(monthToken, each.diPan, gongGua),
        tianPanGongIsMuOrKu:NineYiUtils.checkMuOrKu(monthToken, each.tianPan, gongGua),
        tianPanAnGanIsMuOrKu:NineYiUtils.checkMuOrKu(monthToken, each.tianPanAnGan, gongGua),
        renPanAnGanIsMuOrKu:NineYiUtils.checkMuOrKu(monthToken, each.renPanAnGan, gongGua),
        yinPanAnGanIsMuOrKu:NineYiUtils.checkMuOrKu(monthToken, each.yinGan, gongGua),
    );
  }
  ...
  Map<HouTianGua,EachGong> settleCenterGongJiGong(
    ...
    return _settle(gongRes, houGuaNumberGanMapper, jiGong.getJiGong(yinYangDun, jieQi));
  }

  @Deprecated("使用CenterGongJiGong 内方法代替")
  /// 只寄坤二
  Map<HouTianGua,EachGong> settleAtKun(Map<HouTianGua,EachGong> gongRes, Map<int,TianGan> houGuaNumberGanMapper){
    return _settle(gongRes,houGuaNumberGanMapper,HouTianGua.Kun);
  }
  /// 寄坤二，寄艮八 根据阴阳遁寄宫
  @Deprecated("使用CenterGongJiGong 内方法代替")
  Map<HouTianGua,EachGong> settleAtKunGen(Map<HouTianGua,EachGong> gongRes, Map<int,TianGan> houGuaNumberGanMapper, YinYang yinYangDun){
    HouTianGua atGong = yinYangDun.isYang?HouTianGua.Gen:HouTianGua.Kun;
    return _settle(gongRes,houGuaNumberGanMapper,atGong);
  }
  @Deprecated("使用CenterGongJiGong 内方法代替")
  Map<HouTianGua,EachGong> settleAtFourWei(Map<HouTianGua,EachGong> gongRes, Map<int,TianGan> houGuaNumberGanMapper){
    HouTianGua atGong;
    switch(jieQi.season){
  ...
  @Deprecated("使用CenterGongJiGong 内方法代替")
  Map<HouTianGua,EachGong> settleAtEightGong(Map<HouTianGua,EachGong> gongRes, Map<int,TianGan> houGuaNumberGanMapper){
    HouTianGua atGong;
    switch(jieQi){
  ...
  Map<HouTianGua,EachGong> _settle(
      Map<HouTianGua,EachGong> gongRes,
      Map<int,TianGan> _houGuaNumberGanMapper,
      HouTianGua settleAtGong){
    var diPanJiGong = gongRes[settleAtGong]!;
    diPanJiGong.diPanJiGan = _houGuaNumberGanMapper[5]!;
    // 天盘寄宫，根据地盘寄宫的地盘天干找到天盘所在位置
    var jiGong = gongRes.values.firstWhere((g)=>g.tianPan == diPanJiGong.diPan);
  ...
  Map<TianGan, int> arrangeJu(int numberJu,YinYang yinYangDun){
    ...
    // 确定三奇六仪顺序
    // 阳遁 戊己庚辛壬癸丁丙乙
    List<TianGan> yangDun = const [TianGan.WU,TianGan.JI,TianGan.GENG,TianGan.XIN,TianGan.REN,TianGan.GUI,TianGan.DING,TianGan.BING,TianGan.YI];
    // 阴遁 戊乙丙丁癸壬辛庚己
    List<TianGan> yinDun = const [TianGan.WU,TianGan.YI,TianGan.BING,TianGan.DING, TianGan.GUI,TianGan.REN,TianGan.XIN,TianGan.GENG,TianGan.JI];
    var tianGanSeq = yinYangDun.isYang?yangDun.map((e)=>e):yinDun.map((e)=>e);
    // 构建以numberJu为第一个元素的list,最大数值为9，并将小于numberJu的数字加到后面
    var gongNumberSeq = List.generate(10-numberJu, (index) => index+numberJu).toList()..addAll(originalSeq.sublist(0,numberJu-1));
  ...
  static void checkFuFanYin(ShiJiaQiMen pan){
    ...
    if (!pan.isGanFuYin){
      ...
      pan.isGanFanYin = duiGongTian == kanGong.diPan && duiGongDi == kanGong.tianPan;
    }
