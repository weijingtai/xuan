...
import '../utils/constant_resources_of_qi_men.dart'; // For PlateType
      ...
      children: [
                  ...
                  children: [
                    AnimatedDefaultTextStyle(
                        style: type != PlateType.ZHUAN_PAN
                            ? switcherActivatedStyle.copyWith(
                                fontSize: 24,
                                color: zhuanPanActivatedStyle.color)
                    ...
                    AnimatedDefaultTextStyle(
                        style: type != PlateType.FEI_PAN
                            ? switcherActivatedStyle.copyWith(
                                fontSize: 24, color: feiPanActivatedStyle.color)
        ...
        Container(
            ...
            child: Column(
              children: [
                Row(
                  // JiGong and JieQi/Season selection
                  mainAxisAlignment: MainAxisAlignment.center,
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Column(
                ...
                _buildSettingRow<MonthTokenTypeEnum>(
                    "月令旺衰取法：",
                    monthTokenTypeNotifier,
                    MonthTokenTypeEnum.values,
                    ConstantResourcesOfQiMen.monthTokenHintMapper),
                const SizedBox(height: 12),
                _buildSettingRow<GodWithGongTypeEnum>(
                    "“神”与“宫”旺衰：",
                    godWithGongTypeNotifier,
                    GodWithGongTypeEnum.values,
                    ConstantResourcesOfQiMen.godWithGongTypeMapper),
                const SizedBox(height: 12),
                _buildSettingRow<GongTypeEnum>(
                    "“星”与“宫”旺衰：",
                    starGongTypeNotifier,
                    GongTypeEnum.values,
                    ConstantResourcesOfQiMen.gongTypeMapper),
                const SizedBox(height: 12),
                _buildSettingRow<GongTypeEnum>(
                    "“门”与“宫”旺衰：",
                    doorGongTypeNotifier,
                    GongTypeEnum.values,
                    ConstantResourcesOfQiMen.gongTypeMapper),
                const SizedBox(height: 12),
                _buildSettingRow<GanGongTypeEnum>(
                    "“干”与“宫”旺衰：",
                    ganGongTypeNotifier,
                    GanGongTypeEnum.values,
                    ConstantResourcesOfQiMen.ganGongTypeMapper),
              ],
            )),
        const SizedBox(height: 16),
        ValueListenableBuilder<ArrangeType>(
      ...
      children: [
        ...
        ValueListenableBuilder<T>(
            ...
            builder: (ctx, currentValue, _) {
              return ValueListenableBuilder<PlateType>(
                  // Listen to plateType for dynamic styling
                  valueListenable: plateTypeNotifier,
                  builder: (context, currentPlateType, _) {
                    TextStyle activeStyle =
                        currentPlateType == PlateType.ZHUAN_PAN
                            ? zhuanPanActivatedStyle
                            : feiPanActivatedStyle;
                    return SlideSwitcher(
          ...
          child: ValueListenableBuilder<T>(
              ...
              builder: (ctx, hintKey, _) {
                ...
                if (hintKey is MonthTokenTypeEnum) {
                  Lunar lunar = Lunar.fromDate(
                      DateTime.now()); // Or use selectedDateTimeNotifier.value
                  String monthTokenStr = lunar.getMonthZhi();
                  MonthToken monthToken =
                      DiZhi.getFromValue(monthTokenStr)!.asMonthToken;
                  return RichText(
                      text: TextSpan(
                        ...
                        TextSpan(
                            ...
                            style: TextStyle(
                                color: ConstResourcesMapper
                                    .zodiacZhiColors[monthToken.diZhi]!)),
                        const TextSpan(text: " → "),
                        TextSpan(
                            text: monthToken.majorQi.name,
                            style: TextStyle(
                              color: ConstResourcesMapper
                                  .zodiacGanColors[monthToken.majorQi]!,
                              fontWeight:
                        ...
                        hintKey == MonthTokenTypeEnum.ZHU_QI_NA_GUA
                            ? TextSpan(
                                ...
                                style: TextStyle(
                                    color: ConstResourcesMapper.zodiacGuaColors[
                                        HouTianGua.getGuaByName(
                                            monthToken.majorQi.naJiaGua)]!,
                                    fontWeight: FontWeight.w500))
                            : const TextSpan(text: ""),
                      ]));
                }
                return Text(hintMapper[hintKey] ?? '',
                    style: const TextStyle(
                        fontSize: 12,
                        color: Colors.black54,
                        fontWeight: FontWeight.w300,
  ...
  Widget _buildManuallyJuPanel(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          _buildGanZhiDropdown("年干支", yearGanZhiShakeKey,
              JiaZi.listAll.map((e) => e.name).toList(), onYearJiaZiChanged),
          const SizedBox(width: 12),
          _buildGanZhiDropdown("月干支", monthGanZhiShakeKey,
              JiaZi.listAll.map((e) => e.name).toList(), onMonthJiaZiChanged),
          const SizedBox(width: 12),
          _buildGanZhiDropdown("日干支", dayGanZhiShakeKey,
              JiaZi.listAll.map((e) => e.name).toList(), onDayJiaZiChanged),
          const SizedBox(width: 12),
          _buildGanZhiDropdown("时干支", timeGanZhiShakeKey,
              JiaZi.listAll.map((e) => e.name).toList(), onTimeJiaZiChanged),
          const SizedBox(width: 12),
          ShakeMe(
            ...
            children: [
              ValueListenableBuilder<DateTime?>(
                  ...
                  builder: (ctx, dateTime, _) {
                    ...
                    return Text(dateFormat.format(dateTime ?? DateTime.now()),
