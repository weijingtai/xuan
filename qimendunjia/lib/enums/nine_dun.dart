...
enum NineDunEnum{
  ...
  // 包含天盘、地盘以及天盘寄干，地盘寄干
  static EachGongNineDun? checkNineDunAtEachGong(SixJia xunShou,EachGong gong){
    // 1. 根据天地盘
    NineDunEnum? tianDiPan = checkAndGetNineDun(xunShou, gong.tianPan, gong.diPan, gong.god, gong.door, gong.gongGua);
    NineDunEnum? tianJiGanDiPan;
    if (gong.tianPanJiGan != null){
       tianJiGanDiPan = checkAndGetNineDun(xunShou, gong.tianPanJiGan!, gong.diPan, gong.god, gong.door, gong.gongGua);
    }
    NineDunEnum? tianPanDiPanJiGan;
    if (gong.diPanJiGan != null){
      tianPanDiPanJiGan = checkAndGetNineDun(xunShou, gong.tianPan, gong.diPanJiGan!, gong.god, gong.door, gong.gongGua);
    }
    return EachGongNineDun(tianDiPanGeJuList: tianDiPan, tianPanJiGanWithDiPan: tianJiGanDiPan, tianPanWithDiPanJiGan: tianPanDiPanJiGan);
  }

  static NineDunEnum? checkAndGetNineDun(SixJia xunShou, TianGan tianPan, TianGan diPan,EightGodsEnum god, EightDoorEnum door,HouTianGua gongGua){
  ...
  static bool isWind(TianGan tianPan,EightGodsEnum god, EightDoorEnum door,HouTianGua gongGua){
    return tianPan == TianGan.YI
        && gongGua == HouTianGua.Xun
        && [EightDoorEnum.KAI,EightDoorEnum.XIU,EightDoorEnum.SHENG].contains(door);
  }
  static bool isCloud(TianGan tianPan,TianGan diPan,EightGodsEnum god, EightDoorEnum door){
    return tianPan == TianGan.YI
        && diPan == TianGan.XIN
        && [EightDoorEnum.KAI,EightDoorEnum.XIU,EightDoorEnum.SHENG].contains(door);

  }
  static bool isLoong(TianGan tianPan,TianGan diPan,EightGodsEnum god, EightDoorEnum door,HouTianGua gongGua){
    return tianPan == TianGan.YI
        && [EightDoorEnum.KAI,EightDoorEnum.XIU,EightDoorEnum.SHENG].contains(door)
        && (gongGua == HouTianGua.Kan || diPan == TianGan.GUI);
  }
  static bool isTiger( SixJia xunShou, TianGan tianPan,TianGan diPan,EightGodsEnum god, EightDoorEnum door,HouTianGua gongGua){
    if (xunShou == SixJia.JIA_SHEN_GENG){
      return xunShou.gan == tianPan && gongGua == HouTianGua.Dui && door == EightDoorEnum.KAI;
    }
  ...
  static bool isSky(TianGan tianPan,TianGan diPan,EightGodsEnum god, EightDoorEnum door){
    return tianPan == TianGan.BING
        && diPan == TianGan.DING
        && door == EightDoorEnum.SHENG;
  }
  static bool isEarth(TianGan tianPan,TianGan diPan,EightGodsEnum god, EightDoorEnum door){
    return tianPan == TianGan.YI
        && diPan == TianGan.JI
