...
enum EightDoorEnum {
  ...
  const EightDoorEnum(this.number,
      this.name,
      this.singleCharName,
      this.fiveXing,
      this.originalGong);
  ...
  static List<EightDoorEnum> get listOrderedByGongNumber =>
      [
        XIU, SI, SHANG, DU, CENTER, KAI, JING_W, SHENG , JING_S,
      ];
  ...
  FiveXingWangShuai checkWithMonthToken(MonthToken token) {
    // 同我为旺，生我为相，我生为休、我克为死
  ...
  FiveXingWangShuai checkWithGong(HouTianGua gong) {
    // 同我为旺，生我为相，我生为休、我克为死
    return checkWangShuai(gong.fiveXing);
  }


  FiveXingWangShuai checkWangShuai(FiveXing otherFiveXing) {
    // 同我为旺，生我为相，我生为休、我克为死
    // FiveXingRelationship relationship = FiveXingRelationship.checkRelationship(
...
enum GongAndDoorRelationship{
  ...
  // 根据name返回
  static GongAndDoorRelationship fromName(String name) => values.firstWhere((e) => e.name == name);

  static GongAndDoorRelationship? getRelationship(EightDoorEnum door, HouTianGua gong){
    return _mapper[door] ==null || gong == HouTianGua.Center?null:fromName(_mapper[door]![_gongOrderedList.indexOf(gong)]);
  }
