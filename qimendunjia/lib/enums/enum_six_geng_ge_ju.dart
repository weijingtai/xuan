...
enum EnumSixGengGeJu{
  ...
  static List<EnumSixGengGeJu> checkGengGeForPanel(ShiJiaQiMen pan){
    TianGan yearGan = pan.yearJiaZi.tianGan == TianGan.JIA?pan.xunHeaderTianGan:pan.yearJiaZi.tianGan;
    TianGan monthGan = pan.monthJiaZi.tianGan == TianGan.JIA?pan.xunHeaderTianGan:pan.monthJiaZi.tianGan;
    TianGan timeGan = pan.timeJiaZi.tianGan == TianGan.JIA?pan.xunHeaderTianGan:pan.timeJiaZi.tianGan;
    TianGan dayGan = pan.dayJiaZi.tianGan == TianGan.JIA?pan.xunHeaderTianGan:pan.dayJiaZi.tianGan;

    List<EnumSixGengGeJu> _ganAsTianPan(TianGan diPanGan){
    ...
    List<EnumSixGengGeJu> _ganAsDiPan(TianGan tianPanGan){
      List<EnumSixGengGeJu> res = [];
    ...
    for (var entries in pan.gongMapper.entries) {
      ...
      if (gong.tianPan == TianGan.GENG){
        finalResults.addAll(_ganAsTianPan(gong.diPan));
        // 检查此宫的地盘是否存在 “寄干”
        if (gong.diPanJiGan != null){
          finalResults.addAll(_ganAsTianPan(gong.diPanJiGan!));
        }
      ...
      if (gong.diPan == TianGan.GENG){
        finalResults.addAll(_ganAsDiPan(gong.tianPan));
        if (gong.tianPanJiGan != null){
          finalResults.addAll(_ganAsDiPan(gong.tianPanJiGan!));
        }
      }
