...
enum NineStarsEnum{
  ...
  @JsonValue("天禽")
  QIN(5,"天禽","禽",FiveXing.TU,YinYang.YIN,HouTianGua.Kun); // 没有阴阳是 天芮星与天任星合体，相较于其他九星 属阴
  final String name;
  final int number;
  ...
  static NineStarsEnum fromNumber(int number) => values.firstWhere((e) => e.number == number);
  static NineStarsEnum fromName(String name) => values.firstWhere((e) => e.name == name);
  static NineStarsEnum fromSingleCharName(String name) => values.firstWhere((e) => e.name.startsWith(name));
  ...
  static Map<int,NineStarsEnum> get mapNumberToEnum => Map.fromEntries(values.map((e)=>MapEntry(e.number, e)));
  ...
  /// 使用宫卦五行
  NineStarStatusEnum checkWithGongNeiDiZhi(HouTianGua gong,YinYang yinYangDun,JiaZi shiJiaZi,GongTypeEnum type){
    if (gong.isZheng){
  ...
  /// 根据阴阳遁分阴阳宫，取宫主事地支
  NineStarStatusEnum checkWithGongDiZhiByYinYangDun(HouTianGua gong,YinYang yinYangDun){
    // 主事地支，分阴阳遁 以及 阴阳宫，如：
    // 1.阳遁时：
  ...
  NineStarStatusEnum checkWithGongDiZhiByShiZhi(JiaZi shiJiaZi,HouTianGua gong){
  ...
  NineStarStatusEnum checkWithMonthTokenNaGua(MonthToken token) {
    return checkWithFiveXing(HouTianGua.getGuaByName(token.majorQi.naJiaGua).fiveXing);
  }
  NineStarStatusEnum checkWithFiveXing(FiveXing guaFiveXing) {
    // 《烟波掉搜歌》
    // 与我同行即为相，我生之月诚为旺，废于父母休于财，囚于鬼兮真不妄。
    FiveXingRelationship relationship = FiveXingRelationship.checkRelationship(fiveXing,guaFiveXing)!;
...
enum NineStarStatusEnum{
  ...
  static bool checkWeak(NineStarStatusEnum wangShuai){
    return [NineStarStatusEnum.XIU, NineStarStatusEnum.QIU,NineStarStatusEnum.FEI].contains(wangShuai);
  }
  static bool checkStrong(NineStarStatusEnum wangShuai){
    return [NineStarStatusEnum.WANG, NineStarStatusEnum.XIANG].contains(wangShuai);
  }
}
