...
enum EnumMostPopularGeJu {
  ...
  static EachGongGeJu checkGeJuAtEachGong(JiaZi timeJiaZi, SixJia sixJia,EightDoorEnum zhiShiDoor,EachGong gong){
    ...
    // 1. 处理 天地盘 间的克应
    tianDiPanGeJuList = checkByTianDiPan(sixJia, zhiShiDoor, gong.tianPan, gong.diPan, gong.gongGua, gong.door, gong.god);
    ...
    NineDunEnum? tianDiPanNineDun = NineDunEnum.checkAndGetNineDun(sixJia, gong.tianPan, gong.diPan, gong.god, gong.door, gong.gongGua);
    SanZhaWuJiaEnum? tianDiPanZhaJia = SanZhaWuJiaEnum.checkSanZhaWuJia(gong.tianPan, gong.door, gong.god);
    NineDunEnum? tianPanJiNineDun;
    SanZhaWuJiaEnum? tianPanJiDiPan;
    // 2. 如果天盘有寄干，处理 天盘寄干 与 地盘干的关系
    if (gong.tianPanJiGan != null){
      tianPanJiNineDun = NineDunEnum.checkAndGetNineDun(sixJia, gong.tianPanJiGan!, gong.diPan, gong.god, gong.door, gong.gongGua);
      tianPanJiDiPan = SanZhaWuJiaEnum.checkSanZhaWuJia(gong.tianPanJiGan!, gong.door, gong.god);
      tianPanJiGanWithDiPanList = checkByTianDiPan(sixJia, zhiShiDoor, gong.tianPanJiGan!, gong.diPan, gong.gongGua, gong.door, gong.god);
    ...
    if (gong.diPanJiGan != null){
      tianPanDiPanJiNineDun = NineDunEnum.checkAndGetNineDun(sixJia, gong.tianPan, gong.diPanJiGan!, gong.god, gong.door, gong.gongGua);

      tianPanWithDiPanJiGanList = checkByTianDiPan(sixJia, zhiShiDoor, gong.tianPan, gong.diPanJiGan!, gong.gongGua, gong.door, gong.god);
    ...
    if (gong.tianPanJiGan != null && gong.diPanJiGan != null){
      tianDiJiGanList = checkByTianDiPan(sixJia, zhiShiDoor, gong.tianPanJiGan!, gong.diPanJiGan!, gong.gongGua, gong.door, gong.god);
    }
  ...
  // 奇仪相合
  static EnumMostPopularGeJu? isQiYiHe(TianGan zhiFuGan, TianGan tianPan,TianGan diPan){
    bool itIs = false;
  ...
  // 三奇得使
  static EnumMostPopularGeJu? isSanQiDeShi(TianGan zhiFuGan,EightDoorEnum zhiShiDoor, TianGan tianPan,TianGan diPan,EightDoorEnum door){
    bool itIs = false;
    if (door == zhiShiDoor){
      if (zhiFuGan == diPan){
  ...
  static EnumMostPopularGeJu? isXiangZuo(TianGan zhiFuGan, TianGan tianPan,TianGan diPan){
    bool itIs = false;
    if (zhiFuGan == tianPan){
  ...
  static List<EnumMostPopularGeJu>? checkDayTimeGeJu(JiaZi dayJiaZi, JiaZi timeJiaZi){
  ...
  /// 交泰
  static EnumMostPopularGeJu? isJiaoTai(TianGan tianPan,TianGan diPan,EightDoorEnum door){
    bool itIs = false;
    if ([EightDoorEnum.KAI,EightDoorEnum.XIU,EightDoorEnum.SHENG].contains(door)){
  ...
  // 欢怡
  static EnumMostPopularGeJu? isHuanYi(TianGan zhiFuGan,TianGan tianPan,TianGan diPan){
    bool itIs = false;
  ...
  static EnumMostPopularGeJu? isSanQiShouXing(TianGan tianPan,TianGan diPan,HouTianGua gongGua){
    bool itIs = false;
    if ([TianGan.BING,TianGan.DING].contains(tianPan)&&(
    [TianGan.REN,TianGan.GUI].contains(diPan) || HouTianGua.Kan == gongGua)){
      itIs = true;
  ...
  // 三奇入墓
  static EnumMostPopularGeJu? isSanQiRuMu(TianGan tianPan,TianGan diPan,HouTianGua gongGua){
    bool itIs = false;
    switch (tianPan){
      case TianGan.YI:
        itIs =  [HouTianGua.Qian,HouTianGua.Kun].contains(gongGua);
  ...
  // 三奇贵人升殿
  static EnumMostPopularGeJu? isSanQiShengDian(TianGan tianPan,TianGan diPan,HouTianGua gongGua){
    bool itIs = false;
    switch(tianPan){
      case TianGan.YI:
  ...
  // 玉女守门
  static EnumMostPopularGeJu? isYuNvShouMen(EightDoorEnum zhiShiDoor,TianGan diPan,EightDoorEnum door){
    bool itIs = zhiShiDoor == door && diPan == TianGan.DING;

    return itIs?EnumMostPopularGeJu.Yu_Nv_Shou_Men:null;
  }
  // 三奇之灵
  static EnumMostPopularGeJu? isSanQiZhiLing(TianGan tianPan,TianGan diPan,EightDoorEnum door,EightGodsEnum god){
     bool itIs = [TianGan.YI,TianGan.BING,TianGan.DING].contains(tianPan)
        && [EightDoorEnum.KAI,EightDoorEnum.XIU,EightDoorEnum.SHENG].contains(door)
        && [EightGodsEnum.TAI_YIN,EightGodsEnum.LIU_HE,EightGodsEnum.JIU_DI,EightGodsEnum.JIU_TIAN].contains(god);
    return itIs?EnumMostPopularGeJu.San_Qi_Zhi_Ling:null;
  }
  // 奇游禄位
  static EnumMostPopularGeJu? isQiYouLuWei(TianGan tianPan,HouTianGua gongGua){
    bool itIs = false;
    switch(tianPan){
      case TianGan.YI:
        itIs = gongGua == HouTianGua.Zhen;
