...
enum EightGodsEnum{
  ...
  const EightGodsEnum(this.number,this.name,this.singleCharName,this.fiveXing,this.gong);


  static EightGodsEnum fromNumber(int number) => values.firstWhere((element) => element.number == number);
  static EightGodsEnum fromName(String name) => values.firstWhere((element) => element.name == name);
  static EightGodsEnum fromSingleCharName(String name) => values.firstWhere((e) => e.name.startsWith(name));
  // 1、值符 2、螣蛇 3、太阴 4、六合 5、勾陈 6、太常 7、朱雀 8、九地 9、九天
  // 以上为阳遁时所用九神的排列顺序。
  static List<EightGodsEnum> get feiPanYangDunList =>[
    ZHI_FU,TENG_SHE,TAI_YIN,LIU_HE,GOU_CHEN,TAI_CHANG,ZHU_QI,JIU_DI,JIU_TIAN
  ];
  // 1、值符 2、螣蛇 3、太阴 4、六合 5、白虎 6、 太常 7、玄武 8、九地 9、九天
  // 以上为阴遁时所用九神的排列顺序。
  static List<EightGodsEnum> get feiPanYinDunList =>[
    ZHI_FU,TENG_SHE,TAI_YIN,LIU_HE,BAI_HU,TAI_CHANG,XUAN_WU,JIU_DI,JIU_TIAN
  ];
  ...
  static Map<int,EightGodsEnum> get mapNumberToEnum => Map.fromEntries(values.map((e)=>MapEntry(e.number, e)));

  /// 地盘干的纳卦与八神见的旺衰休囚死
  FiveXingWangShuai checkWangShuaiWithDiPanGanNaGua(TianGan diPanGan){
    return checkWangShuaiWithGua(HouTianGua.getGuaByName(diPanGan.naJiaGua));
  }
  /// 地盘干的纳卦与八神见的旺衰休囚死
  FiveXingWangShuai checkWangShuaiWithGongGua(HouTianGua gong){
    return checkWangShuaiWithGua(gong);
  }
  FiveXingWangShuai checkWangShuaiWithGua(HouTianGua gong){
    FiveXingRelationship relationship = FiveXingRelationship.checkRelationship(fiveXing, gong.fiveXing)!;
    switch(relationship){
      case FiveXingRelationship.SHENG:
        return FiveXingWangShuai.WANG;
      case FiveXingRelationship.TONG:
        return FiveXingWangShuai.XIANG;
      case FiveXingRelationship.XIE:
        return FiveXingWangShuai.XIU;
      case FiveXingRelationship.HAO:
        return FiveXingWangShuai.QIU;
      case FiveXingRelationship.KE:
        return FiveXingWangShuai.SI;
    }
  }
