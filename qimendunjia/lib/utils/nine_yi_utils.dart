...
class NineYiUtils{
  ...
  static bool? checkMuOrKu(MonthToken month,TianGan gan,HouTianGua gong){
    ...
    if (gan == TianGan.BING && HouTianGua.Qian == gong){
      return ![MonthToken.YIN,MonthToken.MAO,MonthToken.SI,MonthToken.WU].contains(month);
    }
    if (gan == TianGan.BING && HouTianGua.Gen == gong){
      return ![MonthToken.YIN,MonthToken.MAO,MonthToken.SI,MonthToken.WU].contains(month);
    }
    if (gan == TianGan.WU && HouTianGua.Qian == gong){
      return ![MonthToken.CHEN,MonthToken.XU,MonthToken.CHOU,MonthToken.WEI].contains(month);
    }
    ...
    if (gan == TianGan.GENG && HouTianGua.Gen == gong){
      return ![MonthToken.SHEN,MonthToken.YOU,MonthToken.CHEN,MonthToken.XU,MonthToken.CHOU,MonthToken.WEI].contains(month);
    }
    ...
    if (gan == TianGan.REN && HouTianGua.Xun == gong){
      return ![MonthToken.SHEN,MonthToken.YOU,MonthToken.HAI,MonthToken.ZI].contains(month);
    }
  ...
  static TwelveZhangSheng qiYiZhangSheng(
      JiaZi shiJiaZi,
      TianGan qi,
      HouTianGua gong,
      YinYang yinYangDun,
      GanGongTypeEnum type){


    if (gong.isZheng){ // 四正宫 坎离震兑
      return TwelveZhangSheng.getZhangShengByTianGanDiZhi(qi, gong.diZhi1);

    }else {// 四余宫 艮巽乾坤

      List<DiZhi> _diZhiList = [gong.diZhi1,gong.diZhi2!];

      // 遇旺则旺，遇墓则墓
      if (type == GanGongTypeEnum.SHI_GAN_YIN_YANG){
        return checkFourWeiGongWangShuaiByShiZhi(shiJiaZi,qi,_diZhiList.toSet());
      }else if (type == GanGongTypeEnum.YIN_YANG_DUN){
        return checkWithYinYangDun(qi,gong,yinYangDun);
      }else{
        return checkFourWeiGongWangShuaiByTraditional(_diZhiList.toSet(),_diZhiList.map((e)=>TwelveZhangSheng.getZhangShengByTianGanDiZhi(qi,e)).toList());
      }
      // 十干阴阳 定旺衰
  ...
  static TwelveZhangSheng checkWithGong(TianGan qi,HouTianGua gong){
    ...
    if (gong.isZheng){ // 四正宫 坎离震兑
      ...
      List<TwelveZhangSheng> _tmpList = _diZhiList.map((e)=>TwelveZhangSheng.getZhangShengByTianGanDiZhi(qi,e)).toList();


      // 遇旺则旺，遇墓则墓
      checkFourWeiGongWangShuaiByTraditional(_diZhiList.toSet(),_tmpList);
      // 十干阴阳 定旺衰
      // checkFourWeiGongWangShuaiByShiZhi（）
  ...
  static TwelveZhangSheng checkWithYinYangDun(TianGan qi,HouTianGua gong,YinYang yinYangDun){
  ...
  static TwelveZhangSheng checkWithSpecificalQi(TianGan qi,HouTianGua gong){
    if ({HouTianGua.Qian,HouTianGua.Kun,HouTianGua.Gen,HouTianGua.Xun}.contains(gong)){
  ...
  static TwelveZhangSheng checkFourWeiGongWangShuaiByTraditional(Set<DiZhi> diZhiList,List<TwelveZhangSheng> zhangShengList){


    // 1. 有墓时先论墓
    if (diZhiList.containsAll({TwelveZhangSheng.SI,TwelveZhangSheng.MU})){
      return TwelveZhangSheng.MU;
    }else if (diZhiList.containsAll({TwelveZhangSheng.GUAN_DAI,TwelveZhangSheng.LIN_GUAN})){
      // 2.有帝旺论帝旺
      return TwelveZhangSheng.DI_WANG;
    }else if (diZhiList.containsAll({TwelveZhangSheng.MU_YU,TwelveZhangSheng.GUAN_DAI})){
      // 3. 有沐浴论沐浴
      return TwelveZhangSheng.MU_YU;
    }else if (diZhiList.containsAll({TwelveZhangSheng.MU_YU,TwelveZhangSheng.LIN_GUAN})){
      // 4. 有临官论临官
      return TwelveZhangSheng.LIN_GUAN;
    }else if (diZhiList.containsAll({TwelveZhangSheng.BING,TwelveZhangSheng.SHUAI})){
      // 5. 有病论病
      return TwelveZhangSheng.BING;
    }else if (diZhiList.containsAll({TwelveZhangSheng.ZHANG_SHEN,TwelveZhangSheng.YANG})){
      // 6. 有长生论长生
      return TwelveZhangSheng.ZHANG_SHEN;
    }else if (diZhiList.containsAll({TwelveZhangSheng.TAI,TwelveZhangSheng.YANG})){
      // 7.有胎论胎
      return TwelveZhangSheng.TAI;
    }
    return TwelveZhangSheng.SI;
  ...
  static TwelveZhangSheng checkFourWeiGongWangShuaiByShiZhi(JiaZi shiJiaZi,TianGan qiGan,Set<DiZhi> diZhiSet){
