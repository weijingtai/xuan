...
class GuaZhongStrategy
    extends StandardCalculationStrategy<GuaZhongStrategyParams,
  ...
  ///
  /// 先天数范围: 1-8
  String _getGuaNameByXiantianNumber(int number) {
    final entry = constants.xianTianGuaNumberMapper.entries
        .firstWhere((entry) => entry.value == number);
    return entry.key;
  }
  ...
  (
    ...
    // 组合主卦名
    final zhuGuaName = upperGuaName + lowerGuaName;

    // 计算互卦
    final huGuaName = gua_utils.guaToHuGua(zhuGuaName);

    // 获取互卦的上下卦先天数
    final huGuaUpperName = huGuaName[0];
    final huGuaLowerName = huGuaName[1];
    ...
    final huGuaTiaoWenNumber_Plan1 = qianWei_Plan1 * 1000 +
        baiWei * 100 +
        huGuaUpperXiantianNumber * 10 +
        huGuaLowerXiantianNumber;

    final huGuaTiaoWenNumber_Plan2 = qianWei_Plan2 * 1000 +
        baiWei * 100 +
        huGuaUpperXiantianNumber * 10 +
        huGuaLowerXiantianNumber;

    final huGuaTiaoWenNumber_Plan3 = qianWei_Plan3 * 1000 +
        baiWei * 100 +
        huGuaUpperXiantianNumber * 10 +
        huGuaLowerXiantianNumber;
