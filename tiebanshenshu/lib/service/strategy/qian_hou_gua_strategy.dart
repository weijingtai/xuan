...
class QianHouGuaStrategyParams extends BaseCalculationParams {
  ...
  String get description =>
      "前后卦取数法计算参数：四柱(${fourZhu.yearGanzhi} ${fourZhu.monthGanzhi} ${fourZhu.dayGanzhi} ${fourZhu.timeGanzhi})，性别($gender)，三元($threeYuan)，节气($birthAfterZhi)";
}
...
class QianHouGuaStrategy
  ...
  BaseNumberModelResult calculate(QianHouGuaStrategyParams params) {
    try {
      // 步骤1-2：获取四柱干支太玄数
      final yearGanNum =
          constants.taixuanGanNumberMapper[params.fourZhu.yearGan]!;
      final yearZhiNum =
          constants.taixuanZhiNumberMapper[params.fourZhu.yearZhi]!;
      final monthGanNum =
          constants.taixuanGanNumberMapper[params.fourZhu.monthGan]!;
      final monthZhiNum =
          constants.taixuanZhiNumberMapper[params.fourZhu.monthZhi]!;
      final dayGanNum =
          constants.taixuanGanNumberMapper[params.fourZhu.dayGan]!;
      final dayZhiNum =
          constants.taixuanZhiNumberMapper[params.fourZhu.dayZhi]!;
      final timeGanNum =
          constants.taixuanGanNumberMapper[params.fourZhu.timeGan]!;
      final timeZhiNum =
          constants.taixuanZhiNumberMapper[params.fourZhu.timeZhi]!;
      ...
      // 根据后天数查卦名
      final qianGuaUpperGua = constants.houTianNumberGuaMapper[yearHouTianNum]!;
      final qianGuaLowerGua =
          constants.houTianNumberGuaMapper[monthHouTianNum]!;
      final qianGuaName = qianGuaUpperGua + qianGuaLowerGua;
      ...
      // 根据后天数查卦名
      final houGuaUpperGua = constants.houTianNumberGuaMapper[dayHouTianNum]!;
      final houGuaLowerGua = constants.houTianNumberGuaMapper[timeHouTianNum]!;
      final houGuaName = houGuaUpperGua + houGuaLowerGua;
      ...
      final model = QianHouGuaBaseNumberModel(
        ...
        // 先后天卦字段（复用字段名，实际是前后卦）
        yearYinYang: params.fourZhu.isYangGanYear ? "阳" : "阴",
