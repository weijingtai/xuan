...
import 'package:datamodels/tiao_wen_datamodel.dart';
...
class MockTiaoWenRepository implements TiaoWenRepository {
  ...
  Future<List<TiaoWenDataModel>> getByIdsWithPageRange(
          {required List<int> ids, required List<int> pageRange, int steps = 1}) =>
      throw UnimplementedError();
  @override
  Future<List<TiaoWenDataModel>> listAll() => throw UnimplementedError();
  @override
  Future<List<TiaoWenDataModel>> search({String? setName, String? contentKeyword}) =>
      throw UnimplementedError();
  @override
  Future<int> getCount() => throw UnimplementedError();
  @override
  Future<List<TiaoWenDataModel>> getAroundById(
          {required int centerId, required int beforeCount, required int afterCount, bool includeCenterItem = true}) =>
      throw UnimplementedError();
  @override
  Future<List<TiaoWenDataModel>> getByIntervalAroundId(
          {required int centerId, required int interval, required int minCount, int? maxRange, bool includeCenterItem = true}) =>
      throw UnimplementedError();
  @override
  Future<List<TiaoWenDataModel>> getByIdRange({required int startId, required int endId}) =>
      throw UnimplementedError();
  @override
  Future<List<TiaoWenDataModel>> getByIdList({required List<int> queryList, bool preserveOrder = false, bool skipNotFound = true}) =>
      throw UnimplementedError();
  @override
  Future<String?> getTiaoWenContentByNumber(int number) => throw UnimplementedError();
}
...
void main() {
  group('LiuQinKaoKeUseCase', () {
    ...
    test('startSessionAndCalculateCandidates should return session with candidates and content', () async {
      // Arrange
      ...
      // Act
      final session = await useCase.startSessionAndCalculateCandidates(gender: gender);
      ...
      final itemWithContent = session.candidateSet!.firstWhere(
        ...
        orElse: () => LiuQinKaoKeSelectionItem(candidate: LiuQinKaoKeCandidate(rawNumber: 0, originKind: OriginKind.innate, changeLineIndex: 0, baseGua: session.candidateSet!.first.candidate.baseGua, huGua: session.candidateSet!.first.candidate.huGua, derivedGua: session.candidateSet!.first.candidate.derivedGua)),
      );
      ...
      expect(session.candidateSet!.any((item) => item.tiaoWenContent != null), isTrue, 
        reason: "At least one candidate should have content if its number matches the mock.");
    });

    test('selectBaseNumbersAndGetFinalList should return session with final list', () async {
      // Arrange
      // 1. 先创建一个 ready 状态的 session
      final initialSession = await useCase.startSessionAndCalculateCandidates(gender: Gender.male);

      final selectedInnate = initialSession.candidateSet!
          .firstWhere((item) => item.candidate.originKind == OriginKind.innate);
      final selectedAcquired = initialSession.candidateSet!
          .firstWhere((item) => item.candidate.originKind == OriginKind.acquired);
