...
class MockCalculationStrategy implements LiuQinKaoKeCalculationStrategy {
  ...
  List<LiuQinKaoKeCandidate> calculateCandidates(
      {required EightChars eightChars, required Gender gender, required bool isYangNianGan}) {
...
void main() {
  ...
  setUp(() {
    ...
    config = TiaoWenListCalculationConfig.listAdd(customList: [96, 192], withSub: true);
    sessionManager = LiuQinKaoKeSessionManager(
        mockSessionRepo, mockStrategy, mockTiaoWenRepo, config);
  });
  ...
  test('completeSelection should advance stage and calculate final list', () async {
    // Arrange
    final initialSession = await sessionManager.start(gender: Gender.male);
    final innateItem = initialSession.candidateSet!.firstWhere((i) => i.candidate.originKind == OriginKind.innate);
    final acquiredItem = initialSession.candidateSet!.firstWhere((i) => i.candidate.originKind == OriginKind.acquired);
    mockTiaoWenRepo.mockContent = {innateItem.candidate.number + 96: 'Derived Content'};
    ...
    expect(finalSession.finalTiaoWenList!.any((item) => item.content == 'Derived Content'), isTrue);
  ...
  test('rollback should revert stage and clear selections', () async {
    ...
    final acquiredItem = initialSession.candidateSet!.firstWhere((i) => i.candidate.originKind == OriginKind.acquired);
    final finalSession = await sessionManager.completeSelection(
        sessionId: initialSession.id, innateItem: innateItem, acquiredItem: acquiredItem);

    // Act
    final rolledBackSession = await sessionManager.rollback(sessionId: finalSession.id);
  ...
  test('resumeMostRecentSession should return the last saved session', () async {
    // Arrange
