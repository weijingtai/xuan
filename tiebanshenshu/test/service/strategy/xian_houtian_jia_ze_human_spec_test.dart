...
void main() {
  ...
  group('步骤1：验证干支配数 - 己酉丙子辛巳戊子', () {
    ...
    test('地支配数应该是 [[4,9], [1,6], [2,7], [1,6]]', () {
      ...
      // 酉: [4,9]
      expect(model.zhiNumList[0].toSet(), equals({4, 9}),
          reason: '酉的配数应该是[4,9]');

      // 子: [1,6]
      expect(model.zhiNumList[1].toSet(), equals({1, 6}),
          reason: '子的配数应该是[1,6]');

      // 巳: [2,7]
      expect(model.zhiNumList[2].toSet(), equals({2, 7}),
          reason: '巳的配数应该是[2,7]');

      // 子: [1,6]
      expect(model.zhiNumList[3].toSet(), equals({1, 6}),
          reason: '子的配数应该是[1,6]');
    });
  });
  ...
  group('步骤3：验证先天卦 - 震为雷', () {
    ...
    test('先天卦基础数应该=3387', () {
      ...
      expect(model.xiantianBaseNumber, equals(3387),
          reason: '先天卦基础数应该是3387');
    });

    test('先天卦条文应该递增96四次: 3387 3483 3579 3675 3771', () {
      print('实际先天卦条文: ${model.xiantianTiaoWenNumbers}');
      expect(model.xiantianTiaoWenNumbers, equals([3387, 3483, 3579, 3675, 3771]),
  ...
  group('步骤4：验证后天卦 - 地雷复', () {
    ...
    test('后天卦基础数应该=2477', () {
      ...
      expect(model.houtianBaseNumber, equals(2477),
          reason: '后天卦基础数应该是2477');
    });

    test('后天卦条文应该递减96四次: 2477 2381 2285 2189 2093', () {
      print('实际后天卦条文: ${model.houtianTiaoWenNumbers}');
      expect(model.houtianTiaoWenNumbers, equals([2477, 2381, 2285, 2189, 2093]),
