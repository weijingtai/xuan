...
void main() {
  ...
  group('步骤3-4：先天卦六爻纳甲和干支和数计算', () {
    ...
    test('先天卦干支和数应该非负（和为10的爻应该为0）', () {
      for (var i = 0; i < model.xiantianYaoSumList.length; i++) {
        expect(model.xiantianYaoSumList[i], greaterThanOrEqualTo(0),
            reason: '第${i + 1}爻的干支和数应该非负（和为10时为0）');
      }
    });

    test('先天卦上三爻和数应该已计算', () {
      expect(model.xiantianUpperSum, greaterThanOrEqualTo(0),
          reason: '上三爻和数应该非负');
    });

    test('先天卦下三爻和数应该已计算', () {
      expect(model.xiantianLowerSum, greaterThanOrEqualTo(0),
          reason: '下三爻和数应该非负');
    });
    ...
    test('先天卦基础数应该=上三爻和数*100+下三爻和数', () {
      ...
      expect(model.xiantianBaseNumber, equals(expectedBaseNumber),
  ...
  group('步骤5-6：后天卦六爻纳甲和干支和数计算', () {
    test('后天卦应该有6个天干纳甲', () {
      expect(model.houtianYaoTianGanList.length, equals(6),
          reason: '后天卦应该有6个爻，每爻配1个天干');
    });

    test('后天卦应该有6个地支纳甲', () {
      expect(model.houtianYaoDiZhiList.length, equals(6),
          reason: '后天卦应该有6个爻，每爻配1个地支');
    });

    test('后天卦应该有6个干支和数', () {
      expect(model.houtianYaoSumList.length, equals(6),
          reason: '后天卦应该有6个爻，每爻有1个干支和数');
    });

    test('后天卦干支和数应该非负（和为10的爻应该为0）', () {
      for (var i = 0; i < model.houtianYaoSumList.length; i++) {
        expect(model.houtianYaoSumList[i], greaterThanOrEqualTo(0),
            reason: '第${i + 1}爻的干支和数应该非负（和为10时为0）');
      }
    });
    ...
    test('后天卦基础数应该=上三爻和数*100+下三爻和数', () {
      ...
      expect(model.houtianBaseNumber, equals(expectedBaseNumber),
          reason: '基础数应该是上三爻和数（千百位）和下三爻和数（十位个位）组合而成');
    });
  });
  ...
  group('Strategy 配置验证', () {
    test('默认条文计算配置应该是递增减96四次（8个数）', () {
      ...
      expect(config.name, contains('递增减96'),
          reason: '默认配置应该是递增减96四次');
    });

    test('策略名称应该正确', () {
      expect(strategy.name, equals('先后天卦六爻干支和数法'),
          reason: '策略名称应该是"先后天卦六爻干支和数法"');
    });

    test('策略描述应该包含关键信息', () {
      expect(strategy.description, contains('六爻纳甲'),
          reason: '描述应该包含"六爻纳甲"');
      expect(strategy.description, contains('干支太玄数'),
          reason: '描述应该包含"干支太玄数"');
    });
  ...
  group('六爻纳甲装配逻辑验证', () {
    test('纳甲天干应该都是有效的天干', () {
      ...
      for (var tianGan in model.xiantianYaoTianGanList) {
        expect(validTianGan, contains(tianGan),
            reason: '纳甲天干$tianGan应该是有效的天干');
      }
    ...
    test('纳甲地支应该都是有效的地支', () {
      ...
      for (var diZhi in model.xiantianYaoDiZhiList) {
        expect(validDiZhi, contains(diZhi),
            reason: '纳甲地支$diZhi应该是有效的地支');
      }
