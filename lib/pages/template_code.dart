...
class _TemplateCodeState extends State<TemplateCode> {
  ...
  void initState() {
    ...
    if (widget.intValueFromArgument != null){
      asyncDoSomeLogicCode(widget.intValueFromArgument!)
          .then((strValue){
    ...
    intValueNotifier.addListener((){
      if (intValueNotifier.value != null){
        asyncDoSomeLogicCode(intValueNotifier.value!)
            .then((strValue) {
    ...
    stringValueNotifier.addListener((){
      if (stringValueNotifier.value != null){
        uiDisplayFinalStringNotifier.value = convertStringToUIString(stringValueNotifier.value!);
      }else{
  ...
  Future<String> loadFromDBOrAssetsFile(int value) async{
    await Future.delayed(Duration(seconds: 1)); // simulate read from assets file or DB or something else
